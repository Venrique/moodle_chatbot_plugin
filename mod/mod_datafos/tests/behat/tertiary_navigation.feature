...
Feature: Users can navigate through the database activity using the tertiary navigation
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | datafos     | Test database name      | Database intro | C1     | data1    |
      | datafos     | Database without fields | Database intro | C1     | data2    |
    And the following "mod_datafos > fields" exist:
      | database | type | name   | description              |
      | data1    | text | field1 | Test field description   |
      | data1    | text | field2 | Test field 2 description |
    And the following "mod_datafos > entries" exist:
    ...
    And I log out
    And I am on the "Test database name" "datafos activity" page logged in as teacher1

  @javascript
  Scenario: The tertiary navigation in the Database page.
    Given I navigate to "Database" in current page administration
    # Teacher: List view.
    And I should not see "List view" in the "datafos-listview-content" "region"
    ...
    And I should not see "Single view" in the "datafos-singleview-content" "region"
    ...
    And I am on the "Database without fields" "datafos activity" page
    And I should not see "Actions"
    # Student without entries: List view.
    And I am on the "Test database name" "datafos activity" page logged in as student1
    And I should not see "Actions"
    # Student without entries: Single view.
    And I set the field "View mode tertiary navigation" to "Single view"
    And I should not see "Actions"
    # Student with entries: Single view.
    But the following "mod_datafos > entries" exist:
      | database | user      | field1               | field2          |
  ...
  Scenario: The tertiary navigation in the Presets page.
    ...
    But I am on the "Database without fields" "datafos activity" page
