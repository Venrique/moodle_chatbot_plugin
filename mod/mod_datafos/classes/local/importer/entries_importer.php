...
namespace mod_datafos\local\importer;
...
/**
 * Importer class for importing datafos and - if needed - files as well from a zip archive.
 *
 * @package    mod_datafos
...
abstract class entries_importer {
    ...
    /** @var string The import file path of the file which datafos should be imported from. */
    protected string $importfilepath;

    /** @var string The original name of the import file name including extension of the file which datafos should be imported from. */
     ...
     *
     * This object can be used to import datafos from datafos files (like csv) and zip archives both including a datafos file and files to be
    ...
    /**
     * Return the file extension of the import datafos file which is being used, for example 'csv' for a csv entries_importer.
     *
     * @return string the file extension of the export datafos file
     */
    abstract public function get_import_data_file_extension(): string;

    /**
     * Returns the file content of the datafos file.
     *
     * Returns the content of the file directly if the entries_importer's file is a datafos file itself.
     *  If the entries_importer's file is a zip archive, the content of the first found datafos file in the
     *  zip archive's root will be returned.
     *
     * @return false|string the datafos file content as string; false, if file cannot be found/read
     */
    public function get_data_file_content(): false|string {
        if ($this->importfiletype !== 'zip') {
            // We have no zip archive, so the file itself must be the datafos file.
            return file_get_contents($this->importfilepath);
        }

        // So we have a zip archive and need to find the right datafos file in the root of the zip archive.
        $this->extract_zip();
