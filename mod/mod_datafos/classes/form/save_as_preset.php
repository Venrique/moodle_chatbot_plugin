...
class save_as_preset extends dynamic_form {
    ...
    protected function definition() {
        ...
        $this->_form->addElement('checkbox', 'overwrite', '', get_string('overrwritedesc', 'datafos'), ['class' => 'hidden']);
    ...
    protected function get_context_for_dynamic_submission(): context {
        ...
        $data = $DB->get_record('datafos', array('id' => $d), '*', MUST_EXIST);
        $course = $DB->get_record('course', array('id' => $data->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('datafos', $data->id, $course->id, null, MUST_EXIST);
    ...
    protected function check_access_for_dynamic_submission(): void {
        ...
        if (!has_capability('mod/datafos:managetemplates', $this->get_context_for_dynamic_submission())) {
            throw new moodle_exception('saveaspresetmissingcapability', 'datafos');
        }
        ...
        if ($action == 'saveaspreset') {
            ...
            $hasfields = $DB->record_exists('data_fields_fos', ['dataid' => $instanceid]);
    ...
    public function process_dynamic_submission(): array {
        ...
        $result = false;
        $errors = [];
        $data = $DB->get_record('datafos', array('id' => $formdata->d), '*', MUST_EXIST);
        $course = $DB->get_record('course', array('id' => $data->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('datafos', $data->id, $course->id, null, MUST_EXIST);
        ...
        try {
            ...
            if (!empty($formdata->oldpresetname)) {
                // Update the name and the description, to save the new datafos.
            ...
            if ($result) {
                ...
                notification::success(get_string('savesuccess', 'mod_datafos', (object)['url' => $previewurl->out()]));
            }
    ...
    /**
     * Load in existing datafos as form defaults.
     *
     * @return void
     */
