 ...
 */
class datafos_field_file extends datafos_field_base {
    var $type = 'file';
    ...
    function display_add_field($recordid = 0, $formdata = null) {
        ...
        $html .= '<div class="mod-datafos-input">';
    ...
    function display_browse_field($recordid, $template) {
        ...
        if ($template==='listtemplate'){
            if ($this->field->name === "File EN") {
                return '&nbsp;<a class="datafos-field-link" href="'.$url.'" >' . s("EN") . '</a> |';
            }

            if ($this->field->name === "File ES") {
                return '&nbsp;<a class="datafos-field-link" href="'.$url.'" >' . s("ES") . '</a> |';
            }

            if ($this->field->name === "File FR") {
                return '&nbsp;<a class="datafos-field-link" href="'.$url.'" >' . s("FR") . '</a> |';
            }

            if ($this->field->name === "File PT") {
                return '&nbsp;<a class="datafos-field-link" href="'.$url.'" >' . s("PT") . '</a>';
            }
        }

        return $icon . '&nbsp;<a class="datafos-field-link" href="'.$url.'" >' . s($name) . '</a>';
    ...
    function update_content($recordid, $value, $name='') {
        ...
        $files = $fs->get_area_files($this->context->id, 'mod_datafos', 'content', $content->id, 'itemid, filepath, filename', false);
        ...
        if (count($files) == 0) {
            ...
            if (count($files) > 1) {
                ...
                debugging('more then one file found in mod_datafos instance {$this->datafos->id} file field (field id: {$this->field->id}) area during update datafos record {$recordid} (content id: {$content->id})', DEBUG_NORMAL);
            }
        }
