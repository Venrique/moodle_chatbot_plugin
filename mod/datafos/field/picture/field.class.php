...
class datafos_field_picture extends datafos_field_base {
    var $type = 'picture';
    var $previewwidth  = 50;
    ...
    function display_add_field($recordid = 0, $formdata = null) {
        ...
        if ($formdata) {
            ...
            if (!$content = $DB->get_record('data_content_fos', array('fieldid' => $this->field->id, 'recordid' => $recordid))) {
                // Quickly make one now!
                $content = new stdClass();
                $content->fieldid  = $this->field->id;
                $content->recordid = $recordid;
                $id = $DB->insert_record('data_content_fos', $content);
                $content = $DB->get_record('data_content_fos', array('id' => $id));
            }
            file_prepare_draft_area($itemid, $this->context->id, 'mod_datafos', 'content', $content->id);
            if (!empty($content->content)) {
                if ($file = $fs->get_file($this->context->id, 'mod_datafos', 'content', $content->id, '/', $content->content)) {
        ...
        $str .= '<noscript>';
        if ($file) {
            $src = file_encode_url($CFG->wwwroot.'/pluginfile.php/', $this->context->id.'/mod_datafos/content/'.$content->id.'/'.$file->get_filename());
        ...
        $str .= '<div class="mod-datafos-input">';
     ...
     *
     * @param stdClass $fieldinput the field input datafos
    ...
    function display_browse_field($recordid, $template) {
        ...
        $imgurl = moodle_url::make_pluginfile_url($this->context->id, 'mod_datafos', 'content', $content->id, '/', $filename);
        ...
        return '<a class="datafos-field-link" href="' . $url->out() . '">' . $img . '</a>';
    }

    function update_field() {
        global $DB, $OUTPUT;

        // Get the old field datafos so that we can check whether the thumbnail dimensions have changed
        $oldfield = $DB->get_record('data_fields_fos', array('id'=>$this->field->id));
        ...
        if ($oldfield && ($oldfield->param4 != $this->field->param4 || $oldfield->param5 != $this->field->param5)) {
            ...
            if ($contents = $DB->get_records('data_content_fos', array('fieldid'=>$this->field->id))) {
                $fs = get_file_storage();
                if (count($contents) > 20) {
                    echo $OUTPUT->notification(get_string('resizingimages', 'datafos'), 'notifysuccess');
                    echo "\n\n";
                ...
                foreach ($contents as $content) {
                    ...
                    if ($thumbfile = $fs->get_file($this->context->id, 'mod_datafos', 'content', $content->id, '/', 'thumb_'.$content->content)) {
                        $thumbfile->delete();
                    }
    ...
    function update_content($recordid, $value, $name='') {
        ...
        $content = $DB->get_record('data_content_fos', array('fieldid' => $this->field->id, 'recordid' => $recordid));
        if (!$content) {
            $content = (object)array('fieldid' => $this->field->id, 'recordid' => $recordid);
            $content->id = $DB->insert_record('data_content_fos', $content);
        }
        ...
        switch ($names[2]) {
            case 'file':
                $fs = get_file_storage();
                file_save_draft_area_files($value, $this->context->id, 'mod_datafos', 'content', $content->id);
                $usercontext = context_user::instance($USER->id);
                ...
                if (count($files) == 0) {
                    ...
                    if (count($files) > 1) {
                        ...
                        debugging('more then one file found in mod_datafos instance {$this->datafos->id} picture field (field id: {$this->field->id}) area during update datafos record {$recordid} (content id: {$content->id})', DEBUG_NORMAL);
                    }
                ...
                $DB->update_record('data_content_fos', $content);
     ...
     *
     * @param stdClass $record the datafos content record the file belongs to
    ...
    /**
     * Provides the necessary code for importing a file when importing the content of a mod_datafos instance.
     *
     * @param int $contentid the id of the mod_datafos content record
