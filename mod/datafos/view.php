...
if ($id) {
    ...
    $record = $DB->get_record('data_records_fos', ['id' => $rid], '*', MUST_EXIST);
    $manager = manager::create_from_data_record($record);
    $cm = $manager->get_coursemodule();
    $course = get_course($cm->course);
} else {   // We must have $d.
    $data = $DB->get_record('datafos', ['id' => $d], '*', MUST_EXIST);
...
if (!empty($advanced)) {
    ...
    list($search_array, $search) = datafos_build_search_array($data, $paging, $search_array);
...
$pageurl = new moodle_url('/mod/datafos/view.php', $urlparams);
...
if (!empty($CFG->enablerssfeeds) && !empty($CFG->data_enablerssfeeds) && $data->rssarticles > 0) {
    ...
    rss_add_http_header($context, 'mod_datafos', $data, $rsstitle);
}
if ($data->csstemplate) {
    $PAGE->requires->css('/mod/datafos/css.php?d='.$data->id);
}
if ($data->jstemplate) {
    $PAGE->requires->js('/mod/datafos/js.php?d='.$data->id, true);
}
...
if ($PAGE->user_allowed_editing() && !$PAGE->theme->haseditswitch) {
    ...
    $editurl = new moodle_url($CFG->wwwroot.'/mod/datafos/view.php', ['id' => $cm->id, 'edit' => $urlediting]);
...
if ($record and !datafos_can_view_record($data, $record, $currentgroup, $canmanageentries)) {
...
/*if (!empty($CFG->enablerssfeeds) && !empty($CFG->data_enablerssfeeds) && $data->rssarticles > 0) {
    ...
    rss_print_link($context->id, $USER->id, 'mod_datafos', $data->id, get_string('rsstype'));
    echo '</div>';
...
if ($multidelete && confirm_sesskey() && $canmanageentries) {
    if ($confirm = optional_param('confirm', 0, PARAM_INT)) {
        foreach ($multidelete as $value) {
            datafos_delete_record($value, $data, $course->id, $cm->id);
        }
    } else {
        $validrecords = array();
        $recordids = array();
...
// If datafos activity closed dont let students in.
// No need to display warnings because activity dates are displayed at the top of the page.
list($showactivity, $warnings) = datafos_get_time_availability_status($data, $canmanageentries);

if ($showactivity) {

    if ($mode == 'asearch') {
        $maxcount = 0;
        datafos_print_preference_form($data, $perpage, $search, $sort, $order, $search_array, $advanced, $mode);
    ...
    /// Check the number of entries required before to view other participant's entries against the number of entries already made (doesn't apply to teachers)
        ...
        if ($groupmode != NOGROUPS) {
            $returnurl = new moodle_url('/mod/datafos/view.php', ['d' => $data->id, 'mode' => $mode, 'search' => s($search),
        ...
        list($records, $maxcount, $totalcount, $page, $nowperpage, $sort, $mode) =
            datafos_search_entries($data, $cm, $context, $mode, $currentgroup, $search, $sort, $order, $page, $perpage, $advanced, $search_array, $record);
        ...
        $actionbar = new \mod_datafos\output\action_bar($data->id, $pageurl);
        ...
        if (empty($records)) {
            ...
            $formurl = new moodle_url('/mod/datafos/view.php', ['d' => $data->id, 'sesskey' => sesskey()]);
            ...
            if ($mode == 'single') { // Single template
                ...
                echo $OUTPUT->box_start('', 'datafos-singleview-content');
                ...
                echo $OUTPUT->box_start('', 'datafos-listview-content');
            ...
            $stickyfooter = new mod_datafos\output\view_footer(
                $manager,
                $totalcount,
                $page,
                $nowperpage,
