...
$url = new moodle_url('/mod/datafos/export.php', array('d' => $d));
$PAGE->set_url($url);
...
if(empty($fieldrecords)) {
    if (has_capability('mod/datafos:managetemplates', $context)) {
        redirect($CFG->wwwroot.'/mod/datafos/field.php?d='.$data->id);
    } else {
...
$mform = new mod_datafos_export_form(new moodle_url('/mod/datafos/export.php', ['d' => $data->id,
    'backto' => $redirectbackto]), $fields, $cm, $data);

if ($mform->is_cancelled()) {
    $redirectbackto = !empty($redirectbackto) ? $redirectbackto :
        new \moodle_url('/mod/datafos/view.php', ['d' => $data->id]);
    ...
    switch ($formdata['exporttype']) {
        case 'csv':
            $exporter = new \mod_datafos\local\exporter\csv_entries_exporter();
            $exporter->set_delimiter_name($formdata['delimiter_name']);
            break;
        case 'ods':
            $exporter = new \mod_datafos\local\exporter\ods_entries_exporter();
            break;
        default:
    ...
    \mod_datafos\local\exporter\utils::data_exportdata($data->id, $fields, $selectedfields, $exporter, $currentgroup, $context,
...
$pagename = get_string('exportentries', 'datafos');
$titleparts = [
    $pagename,
