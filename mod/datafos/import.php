...
$url = new moodle_url('/mod/datafos/import.php');
if ($rid !== 0) {
    $url->param('rid', $rid);
}
if ($fielddelimiter !== '') {
...
if ($id) {
    $url->param('id', $id);
    $PAGE->set_url($url);
    $cm     = get_coursemodule_from_id('datafos', $id, 0, false, MUST_EXIST);
    ...
    $cm     = get_coursemodule_from_instance('datafos', $data->id, $course->id, false, MUST_EXIST);
}
...
echo $OUTPUT->header();
echo $OUTPUT->heading_with_help($pagename, 'uploadrecords', 'mod_datafos');
