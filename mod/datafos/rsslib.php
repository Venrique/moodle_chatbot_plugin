 ...
 */
    function datafos_rss_get_feed($context, $args) {
        global $CFG, $DB;
        ...
        if ($cm) {
            ...
            if ($context->id != $modcontext->id || !has_capability('mod/datafos:viewentry', $modcontext)) {
                return null;
            }
        }
        ...
        $cachedfilepath = rss_get_file_full_name('mod_datafos', $filename);
        ...
        if ( $dontrecheckcutoff > $cachedfilelastmodified && data_rss_newstuff($data, $cachedfilelastmodified)) {
            ...
            foreach ($records as $record) {
                ...
                if (!empty($data->rsstitletemplate)) {
                    ...
                    $item->title   = strip_tags($DB->get_field('data_content_fos', 'content',
                ...
                $item->link = $CFG->wwwroot.'/mod/datafos/view.php?d='.$data->id.'&rid='.$record->id;
            ...
            $header = rss_standard_header($courseshortname . ': ' . format_string($data->name, true, array('context' => $context)),
                                          $CFG->wwwroot."/mod/datafos/view.php?d=".$data->id,
            ...
            if (!empty($header) && !empty($articles) && !empty($footer)) {
                ...
                $status = rss_save_file('mod_datafos', $filename, $rss);
            }
        }
 ...
 */
     ...
     *
     * @param stdClass $data the datafos activity object
    ...
    function data_rss_delete_file($data) {
        ...
        rss_delete_file('mod_datafos', $data);
    }
