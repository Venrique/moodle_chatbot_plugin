...
use mod_datafos\local\importer\preset_importer;
use mod_datafos\local\importer\preset_upload_importer;
use mod_datafos\manager;

require_once('../../config.php');
require_once('lib.php');
require_once($CFG->dirroot.'/mod/datafos/preset_form.php');
...
$url = new moodle_url('/mod/datafos/field.php');
if ($fid !== 0) {
    $url->param('fid', $fid);
}
if ($newtype !== '') {
...
$actionbar = new \mod_datafos\output\action_bar($data->id, $PAGE->url);
...
switch ($mode) {
    ...
    case 'add':    ///add a new field
        ...
        /// Only store this new field if it doesn't already exist.
            ...
            /// Check for arrays and convert to a comma-delimited string
                datafos_convert_arrays_to_strings($fieldinput);

            /// Create a field object to collect and store the datafos safely
                $type = required_param('type', PARAM_FILE);
                $field = datafos_get_field_new($type, $data);
    ...
    case 'delete':    // Delete a field
        if (confirm_sesskey()){
            ...
            if ($confirm = optional_param('confirm', 0, PARAM_INT)) {
                ...
                if ($field = datafos_get_field_from_id($fid, $data)) {
                    $field->delete_field();

                    // Update the templates.
                    datafos_replace_field_in_templates($data, $field->field->name, '');
    ...
    case 'sort':    // Set the default sort parameters
        if (confirm_sesskey()) {
            ...
            $DB->update_record('datafos', $rec);
            redirect($CFG->wwwroot.'/mod/datafos/field.php?d='.$data->id, get_string('changessaved'), 2);
            exit;
        }
        break;
    ...
    case 'usepreset':
        ...
        if (!$importer->needs_mapping() || $action == 'notmapping') {
            $backurl = new moodle_url('/mod/datafos/field.php', ['id' => $cm->id]);
            if ($importer->import(false)) {
                notification::success(get_string('importsuccess', 'mod_datafos'));
            } else {
                notification::error(get_string('cannotapplypreset', 'mod_datafos'));
            }
            redirect($backurl);
        }
...
$PAGE->set_pagetype('mod-datafos-field-' . $newtype);
$titleparts = [
    format_string($data->name),
    format_string($course->fullname),
];
if (($mode == 'new') && (!empty($newtype))) { // Adding a new field.
    ...
    foreach ($fieldrecords as $fieldrecord) {
        ...
        $baseurl = new moodle_url('/mod/datafos/field.php', array(
            'd'         => $data->id,
            'fid'       => $field->field->id,
            'sesskey'   => sesskey(),
        ));
    ...
    echo '<form id="sortdefault" action="'.$CFG->wwwroot.'/mod/datafos/field.php" method="get">';
    echo '<div>';
