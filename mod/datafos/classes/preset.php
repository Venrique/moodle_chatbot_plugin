...
class preset {
    ...
    public static function create_from_storedfile(manager $manager, stored_file $file): ?self {
        if ($file->get_component() != DATAFOS_PRESET_COMPONENT
                || $file->get_filearea() != DATAFOS_PRESET_FILEAREA
                || $file->get_contextid() != DATAFOS_PRESET_CONTEXT) {
            return null;
        }
     ...
     * @return datafos_field_base[]  and array with field objects
     */
    ...
    /**
     * Convert a preset.xml field datafos into field instance.
     *
     ...
     * @return datafos_field_base the field instance
     */
    private function get_field_instance(
        ...
        if (class_exists($classname)) {
            ...
            if ($forpreview && !$newfield->supports_preview()) {
                $newfield = new datafos_field_base($fieldrecord, $instance, $cm);
            }
        } else {
            $newfield = new datafos_field_base($fieldrecord, $instance, $cm);
        }
    ...
    /**
     * Generate a fake field record fomr the preset.xml field datafos.
     *
     * @param SimpleXMLElement $fieldinfo the field xml information
     * @param int $id the field id to use
    ...
    protected static function get_filerecord(string $filename, string $filepath, int $userid): stdClass {
        ...
        $filerecord->contextid = DATAFOS_PRESET_CONTEXT;
        $filerecord->component = DATAFOS_PRESET_COMPONENT;
        $filerecord->filearea = DATAFOS_PRESET_FILEAREA;
        $filerecord->itemid = 0;
    ...
    public static function get_file(string $filepath, string $filename): ?stored_file {
        ...
        $fileexists = $fs->file_exists(
            DATAFOS_PRESET_CONTEXT,
            DATAFOS_PRESET_COMPONENT,
            DATAFOS_PRESET_FILEAREA,
            0,
            $filepath,
            $filename
        );
    ...
    protected function generate_preset_xml(): string {
        ...
        if ($instance->defaultsort > 0 && $sortfield = datafos_get_field_from_id($instance->defaultsort, $instance)) {
