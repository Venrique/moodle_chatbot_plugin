...
class template {
    ...
    /** @var array The mod_datafos fields. */
    protected $fields = [];
    ...
    public function __construct(manager $manager, string $templatecontent, array $options = [], array $fields = null) {
        ...
        $this->canmanageentries = has_capability('mod/datafos:manageentries', $context);
     ...
     * @param bool $form whether the fields should be displayed as form instead of datafos.
    ...
    public static function create_default_template(
        ...
        switch ($templatename) {
            ...
            case 'rsstemplate':
            case 'singletemplate':
                $template = new \mod_datafos\output\defaulttemplate($manager->get_fields(), $templatename, $form);
    ...
    public function add_options(array $options = []) {
        ...
        $baseurl = $options['baseurl'] ?? new moodle_url('/mod/datafos/view.php', ['id' => $cm->id]);
    ...
    private function parse_entry(stdClass $entry): string {
        ...
        $canmanageentry = datafos_user_can_manage_entry($entry, $this->instance, $context);
    ...
    function data_filter_template($template, $patterns, $context) {
        ...
        // To avoid mod_datafos placeholders being processed by filters/clean... we are simply
     ...
     *
     * @param string $result the parsed template with the entry datafos
     * @param stdClass $entry the entry object
     * @return string the final parsed template
     */
    private function post_parse(string $result, stdClass $entry): string {
        if ($this->ratings) {
            $result .= datafos_print_ratings($this->instance, $entry, false);
        }
    ...
    protected function get_tag_edit_replacement(stdClass $entry, bool $canmanageentry): string {
        ...
        $url = new moodle_url('/mod/datafos/edit.php', $this->baseurl->params());
        $url->params([
            'rid' => $entry->id,
            'sesskey' => sesskey(),
    ...
    protected function get_tag_userpicture_replacement(stdClass $entry, bool $canmanageentry): string {
        ...
        // If the record didn't come with user datafos, retrieve the user from database.
    ...
    protected function get_tag_export_replacement(stdClass $entry, bool $canmanageentry): string {
        ...
        $cm = $this->manager->get_coursemodule();
        $context = $this->manager->get_context();
        $canexportall = has_capability('mod/datafos:exportentry', $context);
        $canexportown = has_capability('mod/datafos:exportownentry', $context);
        if (!$canexportall && !(datafos_isowner($entry->id) && $canexportown)) {
            return '';
        }
        ...
        $button->set_callback_options(
            'datafos_portfolio_caller',
            ['id' => $cm->id, 'recordid' => $entry->id],
            'mod_datafos'
        );
        list($formats, $files) = datafos_portfolio_caller::formats($this->fields, $entry);
    ...
    protected function get_tag_approve_replacement(stdClass $entry, bool $canmanageentry): string {
        global $OUTPUT;
        $context = $this->manager->get_context();
        if (!has_capability('mod/datafos:approve', $context) || !$this->instance->approval || $entry->approved) {
            return '';
        }
        $url = new moodle_url($this->baseurl, [
            'approve' => $entry->id,
    ...
    protected function get_tag_tags_replacement(stdClass $entry, bool $canmanageentry): string {
        ...
        return $OUTPUT->tag_list(
            core_tag_tag::get_item_tags('mod_datafos', 'data_records_fos', $entry->id),
            '',
            'datafos-tags'
        );
    }
    ...
    protected function get_tag_otherfields_replacement(stdClass $entry, bool $canmanageentry): string {
        ...
        return $OUTPUT->render_from_template('mod_datafos/fields_otherfields', ['fields' => $fields]);
    }
    ...
    protected function get_tag_actionsmenu_replacement(stdClass $entry, bool $canmanageentry): string {
        ...
        if ($canmanageentry) {
            ...
            $editurl = new moodle_url('/mod/datafos/edit.php', $this->baseurl->params());
            $editurl->params([
                'rid' => $entry->id,
                'sesskey' => sesskey(),
                'backto' => urlencode($backurl->out(false))
        ...
        $context = $this->manager->get_context();
        if (has_capability('mod/datafos:approve', $context) && $this->instance->approval) {
            if ($entry->approved) {
                ...
                $actionmenu->add(new action_menu_link_secondary(
                    $disapproveurl,
                    null,
                    get_string('disapprove', 'mod_datafos')
                ));
            } else {
                ...
                $actionmenu->add(new action_menu_link_secondary(
                    $approveurl,
                    null,
                    get_string('approve', 'mod_datafos')
                ));
            }
        }
        ...
        if (!empty($CFG->enableportfolios)) {
            ...
            if ($canexportall || (datafos_isowner($entry->id) && $canexportown)) {
                ...
                $fields = $this->manager->get_fields();
                list($formats, $files) = datafos_portfolio_caller::formats($fields, $entry);
                $button->set_formats($formats);
     ...
     *
     * @param stdClass|null $processeddata the previous process datafos information.
     * @param int|null $entryid the possible entry id
     * @param stdClass|null $entrydata the entry datafos from a previous form or from a real entry
    ...
    public function parse_add_entry(
        ...
        if (core_tag_tag::is_enabled('mod_datafos', 'data_records_fos')) {
            $patterns[] = "##tags##";
            $replacements[] = datafos_generate_tag_form($entryid);
        }
     ...
     *
     * @param stdClass $processeddata the previous process datafos information.
     * @param datafos_field_base $field the field object
     * @param int|null $entryid the possible entry id
     * @param stdClass|null $entrydata the entry datafos from a previous form or from a real entry
     * @return string the add entry HTML content
     */
