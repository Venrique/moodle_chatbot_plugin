...
use mod_datafos_renderer;
use mod_datafos\event\course_module_viewed;
use mod_datafos\event\template_viewed;
use mod_datafos\event\template_updated;
use moodle_page;
use core_component;
use stdClass;
...
class manager {
    ...
    /** Module name. */
    const MODULE = 'datafos';

    /** The plugin name. */
    const PLUGINNAME = 'mod_datafos';
  ...
  /*  private $instance;*/
   ...
   public $instance;
    ...
    /** @var array the current data_fields_fos records.
     ...
     * @return mod_datafos_renderer the module renderer
     */
    public function get_renderer(?moodle_page $page = null): mod_datafos_renderer {
        global $PAGE;
        $page = $page ?? $PAGE;
     ...
     *
     * @return datafos_field_base[] the field instances.
     */
    ...
    /**
     * Return the field records (the current data_fields_fos records).
     *
     ...
     * @return datafos_field_base the datafos field class instance
     */
    public function get_field(stdClass $fieldrecord): datafos_field_base {
        ...
        $classname = "datafos_field_{$fieldrecord->type}";
        if (!file_exists($filepath)) {
            return new datafos_field_base($fieldrecord, $this->instance, $this->cm);
        }
    ...
    public function get_template(string $templatename, array $options = []): template {
        ...
        if (empty($templatecontent)) {
            $templatecontent = datafos_generate_default_template($instance, $templatename, 0, false, false);
        }
    ...
    public function can_manage_templates(?int $userid = null): bool {
        ...
        return has_capability('mod/datafos:managetemplates', $this->context, $userid);
    }
    ...
    public function update_templates(stdClass $newtemplates): bool {
        ...
        if (isset($record->addtemplate) && !datafos_tags_check($this->instance->id, $record->addtemplate)) {
                return false;
        }
    ...
    public function can_view_preset (preset $preset, ?int $userid = null): bool {
        ...
        if ($presetuserid && $presetuserid != $userid) {
            return has_capability('mod/datafos:viewalluserpresets', $this->context, $userid);
        }
        return true;
    }
    ...
    public function get_available_saved_presets(): array {
        ...
        $canviewall = has_capability('mod/datafos:viewalluserpresets', $this->get_context());
