...
/**
 * Search area for mod_datafos activity entries.
 *
 * @package    mod_datafos
...
class entry extends \core_search\base_mod {
    ...
    /**
     * Returns recordset containing required datafos for indexing database entries.
     *
    ...
    public function get_document_recordset($modifiedfrom = 0, \context $context = null) {
        ...
        list ($contextjoin, $contextparams) = $this->get_context_restriction_sql(
                $context, 'datafos', 'd', SQL_PARAMS_NAMED);
        if ($contextjoin === null) {
            return null;
        }
    ...
    public function get_document($entry, $options = array()) {
        try {
            ...
            debugging('Error retrieving mod_datafos ' . $entry->id . ' document, not all required datafos is available: ' .
                $ex->getMessage(), DEBUG_DEVELOPER);
            return false;
        } catch (\dml_exception $ex) {
            // Notify it as we run here as admin, we should see everything.
            debugging('Error retrieving mod_datafos' . $entry->id . ' document: ' . $ex->getMessage(), DEBUG_DEVELOPER);
            return false;
        }

        // Prepare associative array with datafos from DB.
    ...
    public function attach_files($doc) {
        ...
        foreach ($contents as $content) {
            ...
            $fs = get_file_storage();
            $files = $fs->get_area_files($context->id, 'mod_datafos', 'content', $content->id, 'filename', false);
    ...
    protected function get_entry($entryid) {
        ...
        if (empty($this->entriesdata[$entryid])) {
            $this->entriesdata[$entryid] = $DB->get_record('data_records_fos', array( 'id' => $entryid ), '*', MUST_EXIST);
        }
    ...
    protected function get_fields_for_entries($entry) {
        ...
        $sql = "SELECT dc.*, df.name AS fldname,
                  ...
                  FROM {data_content_fos} dc, {data_fields_fos} df
        ...
        // Filtering out the data_content_fos records having invalid fieldtypes.
        foreach ($contents as $content) {
    ...
    protected function get_field_class_name(string $fieldtype) : ?string {
        ...
        $fieldpath = $CFG->dirroot . '/mod/datafos/field/' . $fieldtype . '/field.class.php';
    ...
    /**
     * Confirms that datafos entries support group restrictions.
     *
     * @return bool True
     */
