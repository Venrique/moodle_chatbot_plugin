...
/**
 * Class for exporting record datafos.
 *
 * @package    mod_datafos
...
class record_exporter extends exporter {
    ...
    protected function get_other_values(renderer_base $output) {
        ...
        $values = array(
            'canmanageentry' => datafos_user_can_manage_entry($this->datafos, $this->related['database'], $this->related['context']),
        );

        if (!empty($this->related['user']) and !empty($this->related['user']->id)) {
            $values['fullname'] = fullname($this->related['user']);
        } else if ($this->datafos->userid) {
            $user = core_user::get_user($this->datafos->userid);
        ...
        $values['tags'] = \core_tag\external\util::get_item_tags('mod_datafos', 'data_records_fos', $this->datafos->id);
