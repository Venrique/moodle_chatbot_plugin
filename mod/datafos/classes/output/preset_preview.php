...
class preset_preview implements templatable, renderable {
    ...
    public function prepare_page(moodle_page $page) {
        ...
        if (!empty($csscontent)) {
            $url = new moodle_url('/mod/datafos/css.php', ['d' => $instance->id, 'preset' => $preset->get_fullname()]);
            $page->requires->css($url);
        }
        $jscontent = $preset->get_template_content('jstemplate');
        if (!empty($jscontent)) {
            $url = new moodle_url('/mod/datafos/js.php', ['d' => $instance->id, 'preset' => $preset->get_fullname()]);
    ...
    public function export_for_template(\renderer_base $output): array {
        ...
        $useurl = new moodle_url('/mod/datafos/field.php');
