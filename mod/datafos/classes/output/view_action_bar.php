...
class view_action_bar implements templatable, renderable {
    ...
    public function export_for_template(\renderer_base $output): array {
        ...
        $activity = $DB->get_record('datafos', ['id' => $this->id], '*', MUST_EXIST);
        $manager = manager::create_from_instance($activity);

        $actionsselect = null;
        // Import entries.
        if (has_capability('mod/datafos:manageentries', $manager->get_context())) {
            $actionsselect = new \action_menu();
            $actionsselect->set_menu_trigger(get_string('actions'), 'btn btn-secondary');

            $importentrieslink = new moodle_url('/mod/datafos/import.php', ['d' => $this->id, 'backto' => $PAGE->url->out(false)]);
        ...
        // Export entries.
        if (has_capability(DATAFOS_CAP_EXPORT, $manager->get_context()) && $this->hasentries) {
            if (!$actionsselect) {
                $actionsselect = new \action_menu();
                $actionsselect->set_menu_trigger(get_string('actions'), 'btn btn-secondary');
            }
            $exportentrieslink = new moodle_url('/mod/datafos/export.php', ['d' => $this->id, 'backto' => $PAGE->url->out(false)]);
            $actionsselect->add(new \action_menu_link(
                $exportentrieslink,
                null,
        ...
        if ($this->mode == '' && !empty($CFG->enableportfolios) && $this->hasentries) {
            if ($manager->can_export_entries()) {
                ...
                if (datafos_portfolio_caller::has_files($activity)) {
                    // No plain HTML.
