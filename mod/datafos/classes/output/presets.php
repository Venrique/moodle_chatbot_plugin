...
class presets implements templatable, renderable {
    ...
    /**
     * Return the preset action menu datafos.
     *
     * @param renderer_base $output The renderer to be used to render the action bar elements.
    ...
    private function get_preset_action_menu(renderer_base $output, $preset, ?int $userid): stdClass {
        ...
        $usepreseturl = new moodle_url('/mod/datafos/preset.php', [
            'action' => 'usepreset',
            'cmid' => $this->cmid,
        ]);
        ...
        $previewpreseturl = new moodle_url('/mod/datafos/preset.php', [
            'fullname' => $preset->get_fullname(),
            'action' => 'preview',
            'id' => $this->cmid,
        ]);
        ...
        if (!$preset->isplugin) {
            // Edit.
            if ($canmanage) {
                $editactionurl = new moodle_url('/mod/datafos/preset.php', [
                    'action' => 'edit',
                    'd' => $this->id,
                ]);
            ...
            // Export.
            $exporturl = new moodle_url('/mod/datafos/preset.php', [
                'presetname' => $preset->name,
                'action' => 'export',
                'd' => $this->id,
            ]);
            ...
            if ($canmanage) {
                ...
                $deleteactionurl = new moodle_url('/mod/datafos/preset.php', [
                    'action' => 'delete',
                    'd' => $this->id,
                ]);
