...
/**
 * Privacy Subsystem implementation for mod_datafos.
 *
 * @package    mod_datafos
...
class provider implements
        // This plugin stores personal datafos.
        \core_privacy\local\metadata\provider,

        // This plugin is capable of determining which users have datafos within it.
    ...
    /**
     * Return the fields which contain personal datafos.
     *
    ...
    public static function get_metadata(collection $collection) : collection {
        $collection->add_database_table(
            'data_records_fos',
            [
                'userid' => 'privacy:metadata:data_records_fos:userid',
                'groupid' => 'privacy:metadata:data_records_fos:groupid',
                'timecreated' => 'privacy:metadata:data_records_fos:timecreated',
                'timemodified' => 'privacy:metadata:data_records_fos:timemodified',
                'approved' => 'privacy:metadata:data_records_fos:approved',
            ],
            'privacy:metadata:data_records_fos'
        );
        $collection->add_database_table(
            'data_content_fos',
            [
                'fieldid' => 'privacy:metadata:data_content_fos:fieldid',
                'content' => 'privacy:metadata:data_content_fos:content',
                'content1' => 'privacy:metadata:data_content_fos:content1',
                'content2' => 'privacy:metadata:data_content_fos:content2',
                'content3' => 'privacy:metadata:data_content_fos:content3',
                'content4' => 'privacy:metadata:data_content_fos:content4',
            ],
            'privacy:metadata:data_content_fos'
        );
    ...
    public static function get_contexts_for_userid(int $userid) : contextlist {
        ...
        // Fetch all datafos records that the user rote.
        $sql = "SELECT c.id
                  FROM {context} c
        ...
        // Fetch all datafos records.
        $ratingquery = \core_rating\privacy\provider::get_sql_join('r', 'mod_datafos', 'entry', 'dr.id', $userid, true);
        $sql = "SELECT c.id
                  FROM {context} c
                  JOIN {course_modules} cm ON cm.id = c.instanceid AND c.contextlevel = :contextlevel
                  JOIN {modules} m ON m.id = cm.module AND m.name = :modname
    ...
    /**
     * Get the list of users who have datafos within a context.
     *
     * @param   userlist    $userlist   The userlist containing the list of users who have datafos in this context/plugin combination.
     *
     */
    public static function get_users_in_context(userlist $userlist) {
        ...
        // Find users with datafos records.
        ...
        \core_comment\privacy\provider::get_users_in_context_from_sql($userlist, 'com', 'mod_datafos', 'database_entry', $context->id);
        ...
        \core_rating\privacy\provider::get_users_in_context_from_sql($userlist, 'rat', 'mod_datafos', 'entry', $sql, $params);
    }
    ...
    protected static function export_data_content($context, $recordobj, $fieldobj, $contentobj) {
        $value = (object)[
            'field' => [
                // Name and description are displayed in mod_datafos without applying format_string().
                'name' => $fieldobj->name,
        ...
        writer::with_context($context)->export_area_files([$recordobj->id, $contentobj->id], 'mod_datafos',
            'content', $contentobj->id);
    }

    /**
     * SQL query that returns all fields from {data_content_fos}, {data_fields_fos} and {data_records_fos} tables
     *
     * @return string
     */
    ...
    /**
     * Export personal datafos for the given approved_contextlist. User and context information is contained within the contextlist.
     *
    ...
    public static function export_user_data(approved_contextlist $contextlist) {
        ...
        $rs = $DB->get_recordset_sql($sql, $contextparams + ['contextlevel' => CONTEXT_MODULE,
                'modname' => 'datafos', 'userid' => $user->id, 'userid1' => $user->id, 'commentarea' => 'database_entry',
                'userid2' => $user->id, 'ratingarea' => 'entry', 'moddata' => 'mod_datafos']);

        $context = null;
        $recordobj = null;
        foreach ($rs as $row) {
            if (!$context || $context->instanceid != $row->cmid) {
                // This row belongs to the different datafos module than the previous row.
                // Export the datafos for the previous module.
                self::export_data($context, $user);
                // Start new datafos module.
    ...
    /**
     * Export one entry in the database activity module (one record in {data_records_fos} table)
     *
     * @param \context $context
     * @param \stdClass $user
    ...
    protected static function export_data_record($context, $user, $recordobj) {
        ...
        \core_tag\privacy\provider::export_item_tags($user->id, $context, [$recordobj->id],
            'mod_datafos', 'data_records_fos', $recordobj->id);
        ...
        \core_comment\privacy\provider::export_comments($context, 'mod_datafos', 'database_entry', $recordobj->id,
        ...
        \core_rating\privacy\provider::export_area_ratings($user->id, $context, [$recordobj->id], 'mod_datafos', 'entry',
            $recordobj->id, $recordobj->userid != $user->id);
    }
    ...
    /**
     * Delete all datafos for all users in the specified context.
     *
    ...
    public static function delete_data_for_all_users_in_context(\context $context) {
        ...
        $sql = "SELECT " . self::sql_fields() . "
                ...
                WHERE cm.id = :cmid
                ORDER BY dr.id";
        $rs = $DB->get_recordset_sql($sql, ['cmid' => $context->instanceid, 'modname' => 'datafos']);
        foreach ($rs as $row) {
    ...
    /**
     * Delete all user datafos for the specified user, in the specified contexts.
     *
    ...
    public static function delete_data_for_user(approved_contextlist $contextlist) {
        ...
        foreach ($contextlist->get_contexts() as $context) {
            ...
            $rs = $DB->get_recordset_sql($sql, ['ctxid' => $context->id, 'contextlevel' => CONTEXT_MODULE,
                'modname' => 'datafos', 'userid' => $user->id]);
            foreach ($rs as $row) {
                self::mark_data_content_for_deletion($context, $row);
                $recordstobedeleted[$row->recordid] = $row->recordid;
            }
        ...
        \core_comment\privacy\provider::delete_comments_for_user($contextlist, 'mod_datafos', 'database_entry');
    ...
    public static function delete_data_for_users(approved_userlist $userlist) {
        ...
        \core_comment\privacy\provider::delete_comments_for_users($userlist, 'mod_datafos', 'database_entry');

        // We do not delete ratings made by users on other records because it may change grades.
    }

    /**
     * Marks a data_record/data_content_fos for deletion
     *
     * Also invokes callback from datafield plugin in case it stores additional datafos that needs to be deleted
     *
     * @param \context $context
    ...
    /**
     * Deletes records marked for deletion and all associated datafos
     *
     * Should be executed after all records were marked by {@link mark_data_content_for_deletion()}
     *
     * Deletes records from data_content_fos and data_records_fos tables, associated files, tags, comments and ratings.
     *
     * @param \context $context
     * @param array $recordstobedeleted list of ids of the datafos records that need to be deleted
     */
    protected static function delete_data_records($context, $recordstobedeleted) {
        ...
        // Delete files.
        get_file_storage()->delete_area_files_select($context->id, 'mod_datafos', 'data_records_fos',
            "IN (SELECT dc.id FROM {data_content_fos} dc WHERE dc.recordid $sql)", $params);
        // Delete from data_content_fos.
        $DB->delete_records_select('data_content_fos', 'recordid ' . $sql, $params);
        // Delete from data_records_fos.
        $DB->delete_records_select('data_records_fos', 'id ' . $sql, $params);
        // Delete tags.
        \core_tag\privacy\provider::delete_item_tags_select($context, 'mod_datafos', 'data_records_fos', $sql, $params);
        // Delete comments.
        \core_comment\privacy\provider::delete_comments_for_all_users_select($context, 'mod_datafos', 'database_entry', $sql, $params);
        // Delete ratings.
        \core_rating\privacy\provider::delete_ratings_select($context, 'mod_datafos', 'entry', $sql, $params);
    }
}
