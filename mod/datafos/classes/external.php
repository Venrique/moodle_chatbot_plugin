...
use mod_datafos\external\database_summary_exporter;
use mod_datafos\external\record_exporter;
use mod_datafos\external\field_exporter;
 ...
 * @since      Moodle 2.9
 */
class mod_datafos_external extends external_api {
    ...
    public static function get_databases_by_courses($courseids = array()) {
        ...
        if (!empty($params['courseids'])) {
            ...
            $databases = get_all_instances_in_courses("datafos", $dbcourses);
    ...
    protected static function validate_database($databaseid) {
        ...
        $database = $DB->get_record('datafos', array('id' => $databaseid), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($database, 'datafos');

        $context = context_module::instance($cm->id);
        self::validate_context($context);
        require_capability('mod/datafos:viewentry', $context);
    ...
    public static function view_database_parameters() {
        return new external_function_parameters(
            array(
                'databaseid' => new external_value(PARAM_INT, 'datafos instance id')
            )
        );
    }

    /**
     * Simulate the datafos/view.php web interface page: trigger events, completion, etc...
     *
     * @param int $databaseid the datafos instance id
    ...
    public static function get_data_access_information($databaseid, $groupid = 0) {
        ...
        $result['canaddentry'] = datafos_user_can_add_entry($database, $groupid, $groupmode, $context);

        // Now capabilities.
        $result['canmanageentries'] = has_capability('mod/datafos:manageentries', $context);
        $result['canapprove'] = has_capability('mod/datafos:approve', $context);

        // Now time access restrictions.
        list($result['timeavailable'], $warnings) = datafos_get_time_availability_status($database, $result['canmanageentries']);

        // Other information.
        $result['numentries'] = datafos_numentries($database);
        $result['entrieslefttoadd'] = datafos_get_entries_left_to_add($database, $result['numentries'], $result['canmanageentries']);
        $result['entrieslefttoview'] = datafos_get_entries_left_to_view($database, $result['numentries'], $result['canmanageentries']);
        $result['inreadonlyperiod'] = datafos_in_readonly_period($database);
    ...
    public static function get_entries($databaseid, $groupid = 0, $returncontents = false, $sort = null, $order = null,
        ...
        list($records, $maxcount, $totalcount, $page, $nowperpage, $sort, $mode) =
            datafos_search_entries($database, $cm, $context, 'list', $groupid, '', $params['sort'], $params['order'],
        ...
        foreach ($records as $record) {
            ...
            $contents = $DB->get_records('data_content_fos', array('recordid' => $record->id));
        ...
        $totalfilesize = 0;
        $fs = get_file_storage();
        $files = $fs->get_area_files($context->id, 'mod_datafos', 'content');
    ...
    public static function get_entry($entryid, $returncontents = false) {
        ...
        if (!datafos_can_view_record($database, $record, $record->groupid, $canmanageentries)) {
            throw new moodle_exception('notapprovederror', 'datafos');
        }

        $related = array('context' => $context, 'database' => $database, 'user' => null);
        if ($params['returncontents']) {
            $related['contents'] = $DB->get_records('data_content_fos', array('recordid' => $record->id));
        } else {
            $related['contents'] = null;
        }
        $exporter = new record_exporter($record, $related);
        ...
        $result = array(
            'entry' => $entry,
            'ratinginfo' => \core_rating\external\util::get_rating_info($database, $context, 'mod_datafos', 'entry', array($record)),
            'warnings' => $warnings
        );
    ...
    public static function get_fields($databaseid) {
        ...
        $fieldinstances = datafos_get_field_instances($database);
     ...
     * @param array $advsearch      advanced search datafos
     * @param str $sort             sort by this field
     * @param int $order            the direction of the sorting
     * @param int $page             page of records to return
     * @param int $perpage          number of records to return per page
    ...
    public static function search_entries($databaseid, $groupid = 0, $returncontents = false, $search = '', $advsearch = [],
        ...
        if (!empty($params['advsearch'])) {
            ...
            list($searcharray, $params['search']) = datafos_build_search_array($database, false, [], $defaults, $fn, $ln);
        } else {
            $advanced = null;
            $searcharray = null;
        }
        ...
        list($records, $maxcount, $totalcount, $page, $nowperpage, $sort, $mode) =
            datafos_search_entries($database, $cm, $context, 'list', $groupid, $params['search'], $params['sort'], $params['order'],
    ...
    public static function approve_entry($entryid, $approve = true) {
        ...
        require_capability('mod/datafos:approve', $context);
    ...
    public static function delete_entry($entryid) {
        ...
        if (datafos_user_can_manage_entry($record, $database, $context)) {
            ...
            throw new moodle_exception('noaccess', 'datafos');
        }
    ...
    public static function add_entry_parameters() {
        return new external_function_parameters(
            array(
                ...
                'groupid' => new external_value(PARAM_INT, 'Group id, 0 means that the function will determine the user group',
                                                   VALUE_DEFAULT, 0),
                'datafos' => new external_multiple_structure(
                    new external_single_structure(
                        array(
                    ...
                    ), 'The fields datafos to be created'
                ),
            )
        );
    }
     ...
     * @param int $databaseid the datafos instance id
     * @param int $groupid (optional) group id, 0 means that the function will determine the user group
     * @param array $data the fields datafos to be created
     * @return array of warnings and status result
     * @since Moodle 3.3
     * @throws moodle_exception
     */
    public static function add_entry($databaseid, $groupid, $data) {
        ...
        $params = array('databaseid' => $databaseid, 'groupid' => $groupid, 'datafos' => $data);
        ...
        if (!datafos_user_can_add_entry($database, $groupid, $groupmode, $context)) {
            throw new moodle_exception('noaccess', 'datafos');
        }

        // Prepare the datafos as is expected by the API.
        $datarecord = new stdClass;
        foreach ($params['datafos'] as $data) {
        ...
        $newentryid = 0;
        if ($processeddata->validated && $recordid = datafos_add_record($database, $groupid)) {
            $newentryid = $recordid;
            // Now populate the fields contents of the new record.
            datafos_add_fields_contents_to_new_record($database, $context, $recordid, $fields, $datarecord, $processeddata);
        }
        ...
        return new external_function_parameters(
            array(
                ...
                'datafos' => new external_multiple_structure(
                    ...
                    ), 'The fields datafos to be updated'
                ),
            )
        );
    }
     ...
     *
     * @param int $entryid the datafos instance id
     * @param array $data the fields datafos to be created
     * @return array of warnings and status result
     * @since Moodle 3.3
     * @throws moodle_exception
    ...
    public static function update_entry($entryid, $data) {
        ...
        $params = array('entryid' => $entryid, 'datafos' => $data);
