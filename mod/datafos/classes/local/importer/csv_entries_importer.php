...
/**
 * CSV entries_importer class for importing datafos and - if needed - files as well from a zip archive.
 *
 * @package    mod_datafos
...
class csv_entries_importer extends entries_importer {
    ...
    /**
     * Declares the entries_importer to use a csv file as datafos file.
     *
     ...
     *
     * @param stdClass $cm Course module of the datafos instance.
    ...
    public function import_csv(stdClass $cm, stdClass $data, string $encoding, string $fielddelimiter): void {
        ...
        if (empty($readcount)) {
            throw new \moodle_exception('csvfailed', 'datafos', "{$CFG->wwwroot}/mod/datafos/edit.php?d={$data->id}");
        } else {
            ...
            if (!empty($errorfield)) {
                throw new \moodle_exception('fieldnotmatched', 'datafos',
                    "{$CFG->wwwroot}/mod/datafos/edit.php?d={$data->id}", $errorfield);
            }
            ...
            while ($record = $cir->next()) {
                ...
                if ($recordid = datafos_add_record($data, 0, $authorid)) {  // Add instance to data_record.
                    foreach ($fields as $field) {
                        ...
                        if (method_exists($field, 'update_content_import')) {
                            ...
                            if ($field->file_import_supported() && $this->importfiletype === 'zip') {
                                ...
                                $contentid = $DB->insert_record('data_content_fos', $content);
                                $field->import_file_value($contentid, $filecontent, $content->content);
                            } else {
                                $DB->insert_record('data_content_fos', $content);
                            }
                        }
                    }
                    ...
                    if (core_tag_tag::is_enabled('mod_datafos', 'data_records_fos') &&
                        ...
                        foreach ($tags as $tag) {
                            ...
                            core_tag_tag::add_item_tag('mod_datafos', 'data_records_fos', $recordid, $context, $tag);
                        }
                    }
