...
$PAGE->set_url('/mod/datafos/index.php', array('id'=>$id));
...
$event = \mod_datafos\event\course_module_instance_list_viewed::create($params);
...
if (! $datas = get_all_instances_in_course("datafos", $course)) {
...
foreach ($datas as $data) {
    ...
    if ($rss && $data->rssarticles > 0) {
        $rsslink = rss_get_link($context->id, $USER->id, 'mod_datafos', $data->id, 'RSS');
    }
    ...
    if ($usesections) {
        ...
        $row = array($printsection, $link, format_module_intro('datafos', $data, $data->coursemodule),
            $numrecords, $numunapprovedrecords);

    } else {
        $row = array($link, format_module_intro('datafos', $data, $data->coursemodule),
            $numrecords, $numunapprovedrecords);
    }
