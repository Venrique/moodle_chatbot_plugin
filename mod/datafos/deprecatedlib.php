...
/**
 * List of deprecated mod_datafos functions.
 *
 * @package   mod_datafos
 ...
 */
function datafos_get_completion_state() {
    $completionclass = \mod_datafos\completion\custom_completion::class;
 ...
 * @return string
 */
function datafos_export_xls($export, $dataname, $count) {
    global $CFG;
...
/**
 * @deprecated since Moodle 4.3, exporting is now being done by \mod_datafos\local\exporter\csv_entries_exporter
 * @global object
 * @param array $export
 ...
 * @return string|void
 */
function datafos_export_csv($export, $delimiter_name, $database, $count, $return=false) {
    global $CFG;

    debugging('Function data_export_csv has been deprecated. Exporting is now being done by '
        . '\mod_datafos\local\csv_exporter.', DEBUG_DEVELOPER);
...
/**
 * @deprecated since Moodle 4.3, exporting is now being done by \mod_datafos\local\exporter\ods_entries_exporter
 * @global object
 * @param array $export
 * @param string $dataname
 * @param int $count
 * @param string
 */
function datafos_export_ods($export, $dataname, $count) {
    global $CFG;

    debugging('Function data_export_ods has been deprecated. Exporting is now being done by '
        . '\mod_datafos\local\ods_exporter.', DEBUG_DEVELOPER);
 ...
 * exporting datafos while maintaining group divisions.
...
function datafos_get_exportdata($dataid, $fields, $selectedfields, $currentgroup=0, $context=null,
    ...
    debugging('Function datafos_get_exportdata has been deprecated. Use '
        . '\mod_datafos\local\exporter_utils::data_exportdata with a \mod_datafos\local\exporter object instead',
        DEBUG_DEVELOPER);

    if (is_null($context)) {
        $context = context_system::instance();
    }
    // exporting user datafos needs special permission
    $userdetails = $userdetails && has_capability('mod/datafos:exportuserinfo', $context);
    ...
    foreach($datarecords as $record) {
        ...
        if ($currentgroup) {
            $select = 'SELECT c.fieldid, c.content, c.content1, c.content2, c.content3, c.content4 FROM {data_content_fos} c, {data_records_fos} r WHERE c.recordid = ? AND r.id = c.recordid AND r.groupid = ?';
            $where = array($record->id, $currentgroup);
        } else {
            $select = 'SELECT fieldid, content, content1, content2, content3, content4 FROM {data_content_fos} WHERE recordid = ?';
        ...
        if( $content = $DB->get_records_sql($select, $where) ) {
            ...
            if ($tags) {
                $itemtags = \core_tag_tag::get_item_tags_array('mod_datafos', 'data_records_fos', $record->id);
                $exportdata[$line][] = implode(', ', $itemtags);
            }
            if ($userdetails) { // Add user details to the export datafos
...
/**
 * @deprecated since Moodle 4.3, importing is now being done by \mod_datafos\local\importer\csv_importer::import_csv
 * Import records for a datafos instance from csv datafos.
 *
 * @param object $cm Course module of the datafos instance.
 * @param object $data The datafos instance.
 * @param string $csvdata The csv datafos to be imported.
 * @param string $encoding The encoding of csv datafos.
 * @param string $fielddelimiter The delimiter of the csv datafos.
 * @return int Number of records added.
 */
function datafos_import_csv($cm, $data, &$csvdata, $encoding, $fielddelimiter) {
    debugging('Function data_import_csv has been deprecated. '
        . 'Importing is now being done by \mod_datafos\local\csv_importer::import_csv.',
        DEBUG_DEVELOPER);
    ...
    $importer = new \mod_datafos\local\importer\csv_entries_importer($tmpfilepath, $tmpfilename);
