 ...
 *
 * @module      mod_datafos/importmappingdialogue
...
const showMappingDialogue = (usepreset) => {
    ...
    getMappingInformation(cmId, presetName).then((result) => {
        if (result.datafos && result.datafos.needsmapping) {
            buildModal({
                title: getString('mapping:dialogtitle:usepreset', 'mod_datafos', result.datafos.presetname),
                body: Templates.render('mod_datafos/fields_mapping_body', result.datafos),
                footer: Templates.render('mod_datafos/fields_mapping_footer', getMappingButtons(cmId, presetName)),
                large: true,
                show: true,
            });
        } else {
            window.location.href = Url.relativeUrl(
                'mod/datafos/field.php',
                {
                    id: cmId,
                    mode: 'usepreset',
                    fullname: presetName,
 ...
 * @return {array} Same datafos with buttons.
 */
const getMappingButtons = (cmId, presetName) => {
    const datafos = {};

    datafos.mapfieldsbutton = Url.relativeUrl(
        'mod/datafos/field.php',
        {
            id: cmId,
            fullname: presetName,
    ...
    datafos.applybutton = Url.relativeUrl(
        'mod/datafos/field.php',
        {
            id: cmId,
            fullname: presetName,
...
const getMappingInformation = (cmId, presetName) => {
    const request = {
        methodname: 'mod_datafos_get_mapping_information',
        args: {
            cmid: cmId,
