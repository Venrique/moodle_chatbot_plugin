 ...
 *
 * @module      mod_datafos/templateseditor
...
const registerResetButton = (mode) => {
    ...
    resetTemplateAction.addEventListener('click', async(event) => {
        ...
        const params = {
            ...
            templatename: await getString(mode, 'mod_datafos'),
        };
        saveCancel(
            getString('resettemplateconfirmtitle', 'mod_datafos'),
            Templates.render('mod_datafos/template_editor_resetmodal', params),
    ...
    toggleTemplateEditor.addEventListener('click', async(event) => {
        ...
        if (enableTemplateEditor) {
            ...
            saveCancel(
                ...
                getString('enabletemplateeditorcheck', 'mod_datafos'),
                getString('editorenable', 'mod_datafos'),
                () => {
                    window.location = relativeUrl('/mod/datafos/templates.php', {d: instanceId, mode: mode, useeditor: true});
                },
                null,
                {triggerElement: event.target}
            );
        } else {
            window.location = relativeUrl('/mod/datafos/templates.php', {d: instanceId, mode: mode, useeditor: false});
        }
    });
};
