 ...
 */
class datafos_portfolio_caller extends portfolio_module_caller_base {

    /** @var int the single record to export */
    protected $recordid;

    /** @var object the record from the datafos table */
    private $data;
    ...
    /**
     * load up the datafos needed for the export
     *
     * @global object $DB
     */
    public function load_data() {
        global $DB, $USER;
        if (!$this->cm = get_coursemodule_from_id('datafos', $this->id)) {
        ...
        foreach ($fieldrecords as $fieldrecord) {
            $tmp = datafos_get_field($fieldrecord, $this->datafos);
            $this->fields[] = $tmp;
        ...
        $this->records = array();
        if ($this->recordid) {
            $tmp = $DB->get_record('data_records_fos', array('id' => $this->recordid));
            ...
            $tmp = $DB->get_records('data_records_fos', $where);
            foreach ($tmp as $t) {
    ...
    public function get_sha1() {
        ...
        // otherwise we're exporting some sort of multipart content so use the datafos
        $str = '';
    ...
    public function prepare_package() {
        ...
        foreach ($this->records  as $key => $record) {
            ...
            $content .= $tmpcontent;
            if ($leapwriter) {
                $entry = new portfolio_format_leap2a_entry('dataentry' . $record->id, $this->datafos->name, 'resource', $tmpcontent);
    ...
    public function check_permissions() {
        if ($this->recordid) {
            if (datafos_isowner($this->recordid)) {
                return has_capability('mod/datafos:exportownentry', context_module::instance($this->cm->id));
            }
            return has_capability('mod/datafos:exportentry', context_module::instance($this->cm->id));
        }
        if ($this->has_export_config() && !$this->get_export_config('mineonly')) {
            return has_capability('mod/datafos:exportallentries', context_module::instance($this->cm->id));
        }
        return has_capability('mod/datafos:exportownentry', context_module::instance($this->cm->id));
    }
    ...
    public function __wakeup() {
        ...
        foreach ($this->fieldtypes as $key => $field) {
            $filepath = $CFG->dirroot . '/mod/datafos/field/' . $field .'/field.class.php';
    ...
    // Replacing special tags (##Edit##, ##Delete##, ##More##)
        ...
        if (empty($this->datafos->singletemplate)) {
            // Use default template if the template is not created.
            $this->datafos->singletemplate = datafos_generate_default_template($this->datafos, 'singletemplate', 0, false, false);
        }

        // actual replacement of the tags
        return array(str_ireplace($patterns, $replacement, $this->datafos->singletemplate), $files);
    }
     ...
     * @param object $record The datafos record object
     *
    ...
    public function has_export_config() {
        ...
        return (empty($this->recordid) // multi-entry export
            ...
            && has_capability('mod/datafos:exportallentries', context_module::instance($this->cm->id))); // they actually have a choice in the matter
    }
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class datafos_file_info_container extends file_info {
    /** @var file_browser */
    protected $browser;
    /** @var stdClass */
    protected $course;
     ...
     */
    public function __construct($browser, $context, $course, $cm, $areas, $filearea) {
        parent::__construct($browser, $context);
        $this->browser = $browser;
        $this->course = $course;
        $this->cm = $cm;
        $this->component = 'mod_datafos'; // Cambiar a 'mod_datafos' si es necesario
    ...
    /**
     * Returns localized visible name of this node
     *
     * @return string
     */
     ...
     * @param bool|int $countonly If false, returns the children; if an int, returns just the count of children but stops counting when $countonly number of children is reached
    ...
    private function get_filtered_children($extensions = '*', $countonly = false, $returnemptyfolders = false) {
        ...
        foreach ($rs as $record) {
            if ($child = $this->browser->get_file_info($this->context, $this->component, $this->filearea, $record->itemid)) {
                $children[] = $child;
            }
     ...
     *
     * @return file_info|null For root
     */
 ...
 * @return void
 */
function datafos_set_events($datafos) {
    global $DB, $CFG;

    require_once($CFG->dirroot.'/calendar/lib.php');

    // Get CMID if not sent as part of $datafos.
    if (!isset($datafos->coursemodule)) {
        $cm = get_coursemodule_from_instance('datafos', $datafos->id, $datafos->course);
        $datafos->coursemodule = $cm->id;
    }
    // Data start calendar events.
    $event = new stdClass();
    $event->eventtype = DATAFOS_EVENT_TYPE_OPEN;
    // The DATAFOS_EVENT_TYPE_OPEN event should only be an action event if no close time was specified.
    $event->type = empty($datafos->timeavailableto) ? CALENDAR_EVENT_TYPE_ACTION : CALENDAR_EVENT_TYPE_STANDARD;
    if ($event->id = $DB->get_field('event', 'id',
        ...
        if ($datafos->timeavailablefrom > 0) {
            ...
            $event->timesort     = $datafos->timeavailablefrom;
            $event->visible      = instance_is_visible('datafos', $datafos);
    ...
    $event->eventtype = DATAFOS_EVENT_TYPE_CLOSE;
    if ($event->id = $DB->get_field('event', 'id',
            array('modulename' => 'datafos', 'instance' => $datafos->id, 'eventtype' => $event->eventtype))) {
        if ($datafos->timeavailableto > 0) {
            ...
            $event->timesort     = $datafos->timeavailableto;
            $event->visible      = instance_is_visible('datafos', $datafos);
            $event->timeduration = 0;
            $calendarevent = calendar_event::load($event->id);
            $calendarevent->update($event, false);
 ...
 * @since  Moodle 3.3
 */
function datafos_get_time_availability_status($data, $canmanageentries = null, $context = null) {
    $open = true;
    $closed = false;
    $warnings = array();
 ...
 * @since  Moodle 3.3
 */
function datafos_require_time_available($data, $canmanageentries = null, $context = null) {

    list($available, $warnings) = datafos_get_time_availability_status($data, $canmanageentries, $context);

    if (!$available) {
        $reason = current(array_keys($warnings));
        throw new moodle_exception($reason, 'datafos', '', $warnings[$reason]);
    }
}
 ...
 * @since  Moodle 3.3
 */
function datafos_get_entries_left_to_add($data, $numentries, $canmanageentries) {
 ...
 * @since  Moodle 3.3
 */
function datafos_get_entries_left_to_view($data, $numentries, $canmanageentries) {
...
/**
 * Returns datafos records tagged with a specified tag.
 *
 * This is a callback used by the tag area mod_datafos/data_records_fos to search for datafos records
 ...
 */
function mod_datafos_get_tagged_records($tag, $exclusivemode = false, $fromctx = 0, $ctx = 0, $rec = true, $page = 0) {
    ...
    $builder = new core_tag_index_builder('mod_datafos', 'data_records_fos', $query, $params, $page * $perpage, $perpage + 1);
    $now = time();
    $entrycount = [];
    $activitygroupmode = [];
    $usergroups = [];
    ...
    while ($item = $builder->has_item_that_needs_access_check()) {
        ...
        if (!has_capability('mod/datafos:viewentry', $context)) {
            $builder->set_accessible($item, false);
            continue;
        }
    ...
    if ($items) {
        ...
        return new core_tag\output\tagindex($tag, 'mod_datafos', 'data_records_fos', $content, $exclusivemode,
 ...
 *
 * @param stdClass $field The field from the 'data_fields_fos' table
 * @param stdClass $entry The entry from the 'data_records_fos' table
 * @return string|null It will return the title of the entry or null if the field type is not available.
 */
function datafos_get_tag_title_for_entry($field, $entry) {
    global $CFG, $DB;
    ...
    $classname = 'datafos_field_' . $field->type;
    $sql = "SELECT dc.*
              FROM {data_content_fos} dc
        INNER JOIN {data_fields_fos} df
                ON dc.fieldid = df.id
             WHERE df.id = :fieldid
 ...
 * @param  str  $search            search for this text in the entry datafos
 ...
 * @param  array  $searcharray     when using advanced search, the advanced datafos to use
 * @param  stdClass  $record       if we just want this record after doing all the access checks
 * @return array the entries found among other datafos related to the search
 * @since  Moodle 3.3
 */

function datafos_search_entries($data, $cm, $context, $mode, $currentgroup, $search = '', $sort = null, $order = null, $page = 0,
    ...
    $approvecap = has_capability('mod/datafos:approve', $context);
    $canmanageentries = has_capability('mod/datafos:manageentries', $context);
    ...
    if (datafos_get_entries_left_to_view($data, $numentries, $canmanageentries)) {
    ...
    if ($sort <= 0 || !($sortfield = datafos_get_field_from_id($sort, $data))) {
    ...
    $recordids = datafos_get_all_recordids($data->id, $initialselect, $initialparams);
    $newrecordids = datafos_get_advance_search_ids($recordids, $searcharray, $data->id);
    $selectdata = $where . $groupselect . $approveselect;

    if (!empty($advanced)) {
        $advancedsearchsql = datafos_get_advanced_search_sql($sort, $data, $newrecordids, $selectdata, $sortorder);
    ...
    if (!$records = $DB->get_records_sql($sqlselect, $allparams, $page * $nowperpage, $nowperpage)) {
        ...
        if ($record) {         // Something was requested so try to show that at least (bug 5132)
            if (datafos_can_view_record($data, $record, $currentgroup, $canmanageentries)) {
 ...
 * @since  Moodle 3.3
 */
function datafos_can_view_record($data, $record, $currentgroup, $canmanageentries) {
    global $USER;
 ...
 * @since  Moodle 3.3
 */
function datafos_get_field_instances($data) {
    global $DB;
 ...
 * @since  Moodle 3.3
 */
function datafos_build_search_array($data, $paging, $searcharray, $defaults = null, $fn = '', $ln = '') {
    global $DB;
    ...
    if (!empty($fields)) {
        foreach ($fields as $field) {
            $searchfield = datafos_get_field_from_id($field->id, $data);
            // Get field datafos to build search sql with.  If paging is false, get from user.
            // If paging is true, get datafos from $searcharray which is obtained from the $SESSION (see line 116).
            if (!$paging && $searchfield->type != 'unknown') {
                ...
                if (isset($searcharray[$field->id])) {
                    $val = $searcharray[$field->id]->datafos;
            ...
            if (!empty($val)) {
                ...
                $searcharray[$field->id]->datafos = $val;
                $vals[] = $val;
            } else {
                // Clear it out.
    ...
    if ($rawtagnames) {
        $searcharray[DATAFOS_TAGS] = new stdClass();
        $searcharray[DATAFOS_TAGS]->params = [];
        $searcharray[DATAFOS_TAGS]->rawtagnames = $rawtagnames;
        $searcharray[DATAFOS_TAGS]->sql = '';
    } else {
        unset($searcharray[DATAFOS_TAGS]);
    }
    ...
    if (!$paging) {
        ...
        $fn = isset($searcharray[DATAFOS_FIRSTNAME]) ? $searcharray[DATAFOS_FIRSTNAME]->datafos : '';
        $ln = isset($searcharray[DATAFOS_LASTNAME]) ? $searcharray[DATAFOS_LASTNAME]->datafos : '';
    }
    if (!empty($fn)) {
        $searcharray[DATAFOS_FIRSTNAME] = new stdClass();
        $searcharray[DATAFOS_FIRSTNAME]->sql    = '';
        $searcharray[DATAFOS_FIRSTNAME]->params = array();
        $searcharray[DATAFOS_FIRSTNAME]->field  = 'u.firstname';
        $searcharray[DATAFOS_FIRSTNAME]->datafos   = $fn;
        $vals[] = $fn;
    } else {
        unset($searcharray[DATAFOS_FIRSTNAME]);
    }
    if (!empty($ln)) {
        $searcharray[DATAFOS_LASTNAME] = new stdClass();
        $searcharray[DATAFOS_LASTNAME]->sql     = '';
        $searcharray[DATAFOS_LASTNAME]->params = array();
        $searcharray[DATAFOS_LASTNAME]->field   = 'u.lastname';
        $searcharray[DATAFOS_LASTNAME]->datafos    = $ln;
        $vals[] = $ln;
    } else {
        unset($searcharray[DATAFOS_LASTNAME]);
    }
 ...
 * @since  Moodle 3.3
 */
function datafos_approve_entry($entryid, $approve) {
    global $DB;

    $newrecord = new stdClass();
    $newrecord->id = $entryid;
    $newrecord->approved = $approve ? 1 : 0;
    $DB->update_record('data_records_fos', $newrecord);
}

/**
 * Populate the field contents of a new record with the submitted datafos.
 ...
 * @param  stdClass $datarecord     the submitted datafos
 * @param  stdClass $processeddata  pre-processed submitted fields
 * @since  Moodle 3.3
 */
function datafos_add_fields_contents_to_new_record($data, $context, $recordid, $fields, $datarecord, $processeddata) {
    global $DB;
    ...
    // Bulk insert the records now. Some records may have no datafos but all must exist.
    $DB->insert_records('data_content_fos', $records);
 ...
 * @since  Moodle 3.3
 */
function datafos_update_record_fields_contents($data, $record, $context, $datarecord, $processeddata) {
    global $DB;
    ...
    $DB->update_record('data_records_fos', $record);
    ...
    $event = \mod_datafos\event\record_updated::create(array(
        'objectid' => $record->id,
        'context' => $context,
