...
class moodle1_mod_data_handler extends moodle1_mod_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path('datafos', '/MOODLE_BACKUP/COURSE/MODULES/MOD/DATA',
                        array(
                            'newfields' => array(
                                'introformat' => 0,
                                'assesstimestart' => 0,
            ...
            new convert_path('datafos_field', '/MOODLE_BACKUP/COURSE/MODULES/MOD/DATA/FIELDS/FIELD')
        );
    }
    ...
    public function process_data($data) {
        ...
        $this->fileman = $this->converter->get_file_manager($contextid, 'mod_datafos');
        ...
        // @todo: user datafos - upgrade content to new file storage
        ...
        //@todo: user datafos - move datafos comments to comments table
        //@todo: user datafos - move datafos ratings to ratings table

        // start writing datafos.xml
        $this->open_xml_writer("activities/data_{$this->moduleid}/datafos.xml");
        $this->xmlwriter->begin_tag('activity', array('id' => $instanceid, 'moduleid' => $this->moduleid,
            'modulename' => 'datafos', 'contextid' => $contextid));
        $this->xmlwriter->begin_tag('datafos', array('id' => $instanceid));
    ...
    public function process_data_record($data) {
        //@todo process user datafos, and define the convert path in get_paths() above.
        //$this->write_xml('record', $data, array('/record/id'));
    }

    /**
     * This is executed when we reach the closing </MOD> tag of our 'datafos' path
     */
    public function on_data_end() {
        // finish writing datafos.xml
        $this->xmlwriter->end_tag('fields');
        $this->xmlwriter->end_tag('datafos');
