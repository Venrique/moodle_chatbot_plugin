...
use mod_datafos\local\importer\preset_existing_importer;
use mod_datafos\manager;

defined('MOODLE_INTERNAL') || die();

class mod_datafos_renderer extends plugin_renderer_base {

    /**
     * Rendering setting and mapping page to import a preset.
     *
     * @param stdClass $datamodule  Database module to import to.
     * @param datafos_preset_importer $importer Importer instance to use for the importing.
     * @return string
     ...
     */
    public function import_setting_mappings($datamodule, datafos_preset_importer $importer) {
        debugging('import_setting_mappings is deprecated. Please use importing_preset instead', DEBUG_DEVELOPER);

        $manager = \mod_datafos\manager::create_from_coursemodule($datamodule);
     ...
     *
     * @param stdClass $datamodule  Database module to import to.
     * @param \mod_datafos\local\importer\preset_importer $importer Importer instance to use for the importing.
     *
     * @return string
     */
    public function importing_preset(stdClass $datamodule, \mod_datafos\local\importer\preset_importer $importer): string {
        ...
        $cancelurl = new moodle_url('/mod/datafos/field.php', ['d' => $datamodule->id]);
     ...
     *
     * @param \mod_datafos\output\fields_action_bar $actionbar
     * @return string The HTML output
     */
    public function render_fields_action_bar(\mod_datafos\output\fields_action_bar $actionbar): string {
        $data = $actionbar->export_for_template($this);
        return $this->render_from_template('mod_datafos/action_bar', $data);
    }
    ...
    public function render_fields_footer(manager $manager): string {
        ...
        $pageurl = new moodle_url('/mod/datafos/templates.php', ['id' => $cm->id]);
        return $this->render_from_template('mod_datafos/fields_footer', [
     ...
     *
     * @param \mod_datafos\output\view_action_bar $actionbar
     * @return string The HTML output
     */
    public function render_view_action_bar(\mod_datafos\output\view_action_bar $actionbar): string {
        $data = $actionbar->export_for_template($this);
        return $this->render_from_template('mod_datafos/view_action_bar', $data);
    }

    /**
     * Renders the action bar for the template page.
     *
     * @param \mod_datafos\output\templates_action_bar $actionbar
     * @return string The HTML output
     */
    public function render_templates_action_bar(\mod_datafos\output\templates_action_bar $actionbar): string {
        $data = $actionbar->export_for_template($this);
        return $this->render_from_template('mod_datafos/templates_action_bar', $data);
    }

    /**
     * Renders the action bar for the preset page.
     *
     * @param \mod_datafos\output\presets_action_bar $actionbar
     * @return string The HTML output
     */
    public function render_presets_action_bar(\mod_datafos\output\presets_action_bar $actionbar): string {
        $data = $actionbar->export_for_template($this);
        return $this->render_from_template('mod_datafos/presets_action_bar', $data);
    }

    /**
     * Renders the presets table in the preset page.
     *
     * @param \mod_datafos\output\presets $presets
     * @return string The HTML output
     */
    public function render_presets(\mod_datafos\output\presets $presets): string {
        $data = $presets->export_for_template($this);
        return $this->render_from_template('mod_datafos/presets', $data);
    }

    /**
     * Renders the default template.
     *
     * @param \mod_datafos\output\defaulttemplate $template
     * @return string The HTML output
     */
    public function render_defaulttemplate(\mod_datafos\output\defaulttemplate $template): string {
     ...
     *
     * @param \mod_datafos\manager $manager The manager instance.
     *
     * @return string The HTML output
     */
    public function render_database_zero_state(\mod_datafos\manager $manager): string {
        $actionbar = new \mod_datafos\output\zero_state_action_bar($manager);
        $data = $actionbar->export_for_template($this);
        if (empty($data)) {
            // No actions for the user.
            $data['title'] = get_string('activitynotready');
            $data['intro'] = get_string('comebacklater');
            $data['noitemsimgurl'] = $this->output->image_url('noentries_zero_state', 'mod_datafos')->out();
        } else {
            $data['title'] = get_string('startbuilding', 'mod_datafos');
            $data['intro'] = get_string('createactivity', 'mod_datafos');
            $data['noitemsimgurl'] = $this->output->image_url('view_zero_state', 'mod_datafos')->out();
        }

        return $this->render_from_template('mod_datafos/zero_state', $data);
    }
     ...
     * @return string The HTML output
     */
    public function render_empty_database(\mod_datafos\manager $manager): string {
        $actionbar = new \mod_datafos\output\empty_database_action_bar($manager);
        $data = $actionbar->export_for_template($this);
        $data['noitemsimgurl'] = $this->output->image_url('view_zero_state', 'mod_datafos')->out();

        return $this->render_from_template('mod_datafos/view_noentries', $data);
    }
     ...
     * @return string The HTML output
     */
    public function render_fields_zero_state(\mod_datafos\manager $manager): string {
        $data = [
            'noitemsimgurl' => $this->output->image_url('fields_zero_state', 'mod_datafos')->out(),
            'title' => get_string('nofields', 'mod_datafos'),
            'intro' => get_string('createfields', 'mod_datafos'),
            ];
        if ($manager->can_manage_templates()) {
            $actionbar = new \mod_datafos\output\action_bar($manager->get_instance()->id, $this->page->url);
     ...
     * @return string The HTML output
     */
    public function render_templates_zero_state(\mod_datafos\manager $manager): string {
        $actionbar = new \mod_datafos\output\zero_state_action_bar($manager);
        $data = $actionbar->export_for_template($this);
        $data['title'] = get_string('notemplates', 'mod_datafos');
        $data['intro'] = get_string('createtemplates', 'mod_datafos');
        $data['noitemsimgurl'] = $this->output->image_url('templates_zero_state', 'mod_datafos')->out();
