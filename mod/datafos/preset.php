...
use mod_datafos\output\action_bar;
use mod_datafos\output\preset_preview;
...
$url = new moodle_url('/mod/datafos/preset.php', array('d' => $data->id));
...
$PAGE->requires->js_call_amd('mod_datafos/deletepreset', 'init');
...
if ($action == 'importzip') {
    ...
    core\notification::success(get_string('importsuccess', 'mod_datafos'));
    redirect(new moodle_url('/mod/datafos/field.php', ['id' => $cm->id]));
    exit(0);
}
...
$actionbar = new \mod_datafos\output\action_bar($data->id, $url);
echo $actionbar->get_presets_action_bar();
$presets = new \mod_datafos\output\presets($manager, $presets, new \moodle_url('/mod/datafos/field.php'), true);
