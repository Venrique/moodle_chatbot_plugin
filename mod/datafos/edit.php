...
$url = new moodle_url('/mod/datafos/edit.php');
...
if (!has_capability('mod/datafos:manageentries', $context)) {
    if ($rid) {
        ...
        if (!datafos_user_can_manage_entry($record, $data, $context)) {
            throw new \moodle_exception('noaccess', 'datafos');
        }
    } else if (!datafos_user_can_add_entry($data, $currentgroup, $groupmode, $context)) {
...
// Process incoming datafos for adding/updating records.

// Keep track of any notifications ad submitted datafos.
$processeddata = null;
$datarecord = data_submitted() ?: null;

// Process the submitted form.
if ($datarecord && confirm_sesskey()) {
    // Validate the form to ensure that enough datafos was submitted.
    $fields = $manager->get_field_records();
    $processeddata = datafos_process_submission($data, $fields, $datarecord);
    ...
    if ($processeddata->validated) {
        if ($rid) {
            ...
            if ($recordid) {
                ...
                datafos_add_fields_contents_to_new_record($data, $context, $recordid, $fields, $datarecord, $processeddata);
            }
        }

        if ($recordid) {
            core_tag_tag::set_item_tags('mod_datafos', 'data_records_fos', $recordid, $context, $tags);

            if (!empty($datarecord->saveandadd)) {
                // User has clicked "Save and add another". Reset all of the fields.
                $datarecord = null;
            } else {
                $viewurl = new moodle_url('/mod/datafos/view.php', [
                    'd' => $data->id,
                    'rid' => $recordid,
                ]);
                redirect($viewurl);
...
groups_print_activity_menu($cm, $CFG->wwwroot.'/mod/datafos/edit.php?d='.$data->id);

// Form goes here first in case add template is empty.
echo '<form enctype="multipart/form-datafos" action="edit.php" method="post">';
echo '<div>';
...
if (empty($redirectbackto)) {
    $redirectbackto = new \moodle_url('/mod/datafos/view.php', ['id' => $cm->id]);
}
...
if (!$rid && ((!$data->maxentries) ||
    has_capability('mod/datafos:manageentries', $context) ||
