...
class externallib_test extends externallib_advanced_testcase {
    ...
    public function setUp(): void {
        ...
        $this->database = $this->getDataGenerator()->create_module('datafos', array('course' => $this->course->id));
        $this->context = \context_module::instance($this->database->cmid);
        $this->cm = get_coursemodule_from_instance('datafos', $this->database->id);
     ...
     *
     * @return \datafos_field_base
     */
    protected function add_test_field(): \datafos_field_base {
     ...
     */
    public function test_mod_datafos_get_databases_by_courses() {
        global $DB;
        ...
        $database1 = self::getDataGenerator()->create_module('datafos', $record);

        // Second database.
        $record = new \stdClass();
        $record->introformat = FORMAT_HTML;
        $record->course = $course2->id;
        $database2 = self::getDataGenerator()->create_module('datafos', $record);
    ...
    public function test_view_database_no_capabilities() {
        ...
        assign_capability('mod/datafos:view', CAP_PROHIBIT, $this->studentrole->id, $this->context->id);
    ...
    public function test_view_database() {
        ...
        $this->assertInstanceOf('\mod_datafos\event\course_module_viewed', $event);
        $this->assertEquals($this->context, $event->get_context());
        $moodledata = new \moodle_url('/mod/datafos/view.php', array('id' => $this->cm->id));
    ...
    public function test_get_data_access_information_student() {
        ...
        $DB->update_record('datafos', $this->database);
    ...
    public function populate_database_with_entries() {
        ...
        // Force approval.
        $DB->set_field('datafos', 'approval', 1, array('id' => $this->database->id));
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_datafos');
        ...
        $DB->set_field('data_records_fos', 'approved', 1, ['id' => $entry11]);
        $DB->set_field('data_records_fos', 'approved', 1, ['id' => $entry12]);
        $DB->set_field('data_records_fos', 'approved', 1, ['id' => $entry14]);
        $DB->set_field('data_records_fos', 'approved', 1, ['id' => $entry21]);
    ...
    public function test_get_entries() {
        ...
        datafos_generate_default_template($this->database, 'listtemplate', 0, false, true); // Generate a default list template.
    ...
    public function test_get_entry_separated_groups() {
        ...
        // Retrieve contents.
        datafos_generate_default_template($this->database, 'singletemplate', 0, false, true);
    ...
    public function test_search_entries() {
        ...
        $this->setUser($this->student2);
        $result = mod_data_external::search_entries($this->database->id, 0, false, 'text', [], DATAFOS_APPROVED, 'ASC');
        $result = external_api::clean_returnvalue(mod_data_external::search_entries_returns(), $result);
        ...
        // Pagination.
        $this->setUser($this->teacher);
        $result = mod_data_external::search_entries($this->database->id, 0, false, 'text', [], DATAFOS_TIMEADDED, 'ASC', 0, 2);
        $result = external_api::clean_returnvalue(mod_data_external::search_entries_returns(), $result);
        ...
        $field = $DB->get_record('data_fields_fos', array('type' => 'url'));
        $advsearch = [
        ...
        // Combined search.
        $field2 = $DB->get_record('data_fields_fos', array('type' => 'number'));
        $advsearch = [
            ['name' => 'f_' . $field->id , 'value' => 'sampleurl'],
    ...
    public function test_approve_entry() {
        ...
        $this->setUser($this->teacher);
        $this->assertEquals(0, $DB->get_field('data_records_fos', 'approved', array('id' => $entry13)));
        ...
        $this->assertEquals(1, $DB->get_field('data_records_fos', 'approved', array('id' => $entry13)));
    }
    ...
    public function test_unapprove_entry() {
        ...
        $this->setUser($this->teacher);
        $this->assertEquals(1, $DB->get_field('data_records_fos', 'approved', array('id' => $entry11)));
        $result = mod_data_external::approve_entry($entry11, false);
        $result = external_api::clean_returnvalue(mod_data_external::approve_entry_returns(), $result);
        $this->assertEquals(0, $DB->get_field('data_records_fos', 'approved', array('id' => $entry11)));
    }
    ...
    public function test_delete_entry_as_teacher() {
        ...
        $this->assertEquals(0, $DB->count_records('data_records_fos', array('id' => $entry11)));

        // Entry in other group.
        $result = mod_data_external::delete_entry($entry21);
        $result = external_api::clean_returnvalue(mod_data_external::delete_entry_returns(), $result);
        $this->assertEquals(0, $DB->count_records('data_records_fos', array('id' => $entry21)));
    }
    ...
    public function test_add_entry() {
        ...
        foreach ($result['entry']['contents'] as $content) {
            ...
            $dbcontent = $DB->get_record('data_content_fos', array('fieldid' => $field->id, 'recordid' => $newentryid));
        ...
        // Now, try to add another entry but removing some required datafos.
        ...
        $this->assertEquals(get_string('errormustsupplyvalue', 'datafos'), $result['fieldnotifications'][0]['notification']);
    }
    ...
    public function test_add_entry_empty_form() {
        ...
        $this->assertEquals(get_string('emptyaddform', 'datafos'), $result['generalnotifications'][0]);
    }
    ...
    public function test_add_entry_read_only_period() {
        ...
        $this->setUser($this->student1);
        $this->expectExceptionMessage(get_string('noaccess', 'datafos'));
        $this->expectException('moodle_exception');
    ...
    public function test_update_entry() {
        ...
        foreach ($result['entry']['contents'] as $content) {
            $field = $fields[$content['fieldid']];
            // Stored content same that the one retrieved by WS.
            $dbcontent = $DB->get_record('data_content_fos', array('fieldid' => $field->id, 'recordid' => $entry11));
            $this->assertEquals($dbcontent->content, $content['content']);
        ...
        // Now, try to update the entry but removing some required datafos.
        unset($newentrydata[0]);
        $result = mod_data_external::update_entry($entry11, $newentrydata);
        $result = external_api::clean_returnvalue(mod_data_external::update_entry_returns(), $result);
    ...
    /**
     * Test update_entry sending empty datafos.
     */
    ...
    public function test_get_entry_rating_information() {
        ...
        $DB->set_field('datafos', 'assessed', RATING_AGGREGATE_SUM, array('id' => $this->database->id));
        $DB->set_field('datafos', 'scale', 100, array('id' => $this->database->id));
        list($entry11, $entry12, $entry13, $entry14, $entry21) = self::populate_database_with_entries();
