@mod @mod_datafos
Feature: Users can view and manage datafos presets
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | datafos     | Mountain landscapes | n     | C1     | data1    |
    And the following "mod_datafos > presets" exist:
  ...
  Scenario: Admins can delete saved presets
    Given I am on the "Mountain landscapes" "datafos activity" page logged in as admin
  ...
  Scenario: Teachers can see and use presets
    Given I am on the "Mountain landscapes" "datafos activity" page logged in as teacher1
    When I follow "Presets"
    Then I should see "Choose a preset to use as a starting point."
    And I should see "Image gallery"
  ...
  Scenario: Only users with the viewalluserpresets capability can see presets created by other users
    Given the following "permission override" exists:
      | role         | editingteacher              |
      | capability   | mod/datafos:viewalluserpresets |
      | permission   | Prohibit                    |
      | contextlevel | System                      |
      | reference    |                             |
    When I am on the "Mountain landscapes" "datafos activity" page logged in as teacher1
  ...
  Scenario: Teachers can save presets
    Given the following "mod_datafos > fields" exist:
      | database | type | name            | description            |
      | data1    | text | Test field name | Test field description |
    And I am on the "Mountain landscapes" "datafos activity" page logged in as teacher1
  ...
  Scenario: Teachers can edit presets and overwrite them if they are the authors
    Given the following "mod_datafos > preset" exists:
      | database    | data1                                |
  ...
  Scenario: Teachers can preview a saved preset from the notification
    Given the following "mod_datafos > fields" exist:
      | database | type | name            | description            |
      | data1    | text | Test field name | Test field description |
    And the following "mod_datafos > templates" exist:
      | database | name            |
  ...
  Scenario Outline: Admins and Teachers can load a preset from a file
    Given I am on the "Mountain landscapes" "datafos activity" page logged in as <user>
    When I follow "Presets"
    Then I click on "Actions" "button"
    And I choose "Import preset" in the open action menu
    And I upload "mod/datafos/tests/fixtures/image_gallery_preset.zip" file to "Preset file" filemanager
