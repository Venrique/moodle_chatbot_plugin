...
/**
 * Behat datafos generator for mod_datafos.
 *
 * @package   mod_datafos
 * @category  test
...
class behat_mod_data_generator extends behat_generator_base {
    ...
    protected function get_database_id(string $idnumber): int {
        $cm = $this->get_cm_by_activity_name('datafos', $idnumber);
     ...
     * Add an entry.
     *
     * @param array $data Entry datafos.
     */
    public function process_entry(array $data): void {
        global $DB;

        $database = $DB->get_record('datafos', ['id' => $data['databaseid']], '*', MUST_EXIST);
     ...
     * Add a field.
     *
     * @param array $data Field datafos.
     */
     ...
     * Add a template.
     *
     * @param array $data Template datafos.
     */
    public function process_template(array $data): void {
        ...
        if (empty($data['content'])) {
            datafos_generate_default_template($database, $data['name']);
        } else {
     ...
     * Saves a preset.
     *
     * @param array $data Preset datafos.
     */
    protected function process_preset(array $data): void {
        global $DB;

        $instance = $DB->get_record('datafos', ['id' => $data['databaseid']], '*', MUST_EXIST);

        $this->get_data_generator()->create_preset($instance, (object) $data);
    }

    /**
     * Get the module datafos generator.
     *
     * @return mod_data_generator Database datafos generator.
     */
