...
class locallib_test extends \advanced_testcase {
     ...
     */
    public function test_datafos_search_entries() {
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        $titlefield = $this->getDataGenerator()->get_plugin_generator('mod_datafos')->create_field(
            (object) [
                'name' => 'title',
                'type' => 'text',
                'required' => 1
            ...
            $module);
        $captionfield = $this->getDataGenerator()->get_plugin_generator('mod_datafos')->create_field(
            (object) [
                'name' => 'caption',
                'type' => 'text',
                'required' => 1
            ...
            $module);
        $this->getDataGenerator()->get_plugin_generator('mod_datafos')->create_entry($module, [
        ...
        $cm = get_coursemodule_from_id('datafos', $module->cmid);
        ...
        list($records, $maxcount, $totalcount, $page, $nowperpage, $sort, $mode) =
            datafos_search_entries($module, $cm, \context_course::instance($course->id), 'list', 0);
        $this->assertCount(2, $records);
        // Search for entries for "caption" we should return only one of them.
        list($records, $maxcount, $totalcount, $page, $nowperpage, $sort, $mode) =
            datafos_search_entries($module, $cm, \context_course::instance($course->id), 'list', 0, 'caption');
        ...
        list($searcharray, $searchtext) = datafos_build_search_array($module, false, [], $defaults, $fn, $ln);
        list($records, $maxcount, $totalcount, $page, $nowperpage, $sort, $mode) =
            datafos_search_entries($module, $cm, \context_course::instance($course->id), 'list', 0, $searchtext,
    ...
    private function assert_record_entries_contains($records, $fieldid, $content) {
        ...
        foreach ($records as $record) {
            $fieldscontent = $DB->get_records('data_content_fos', ['recordid' => $record->id]);
