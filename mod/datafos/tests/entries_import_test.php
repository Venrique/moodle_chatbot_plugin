...
use dml_exception;
use mod_datafos\local\importer\csv_entries_importer;
...
class entries_import_test extends \advanced_testcase {
    ...
    /**
     * Test uploading entries for a datafos instance without userdata.
     *
    ...
    /**
     * Test uploading entries for a datafos instance with userdata.
     *
    ...
    /**
     * Test uploading entries for a datafos instance with userdata and a defined field 'Username'.
     *
     * This should test the corner case, in which a user has defined a datafos fields, which has the same name
    ...
    /**
     * Test uploading entries for a datafos instance with a field 'Username' but only one occurrence in the csv file.
     *
     * This should test the corner case, in which a user has defined a datafos fields, which has the same name
     ...
     *
     * @covers \mod_datafos\local\importer\entries_importer
     * @covers \mod_datafos\local\importer\csv_entries_importer
     * @return void
     */
    public function test_import_with_files(): void {
        ...
        // stored in the mod_datafos record in the field 'filefield'.
        ...
        $filefield = datafos_get_field_from_name('filefield', $data);
        ...
        // stored in the mod_datafos record in the field 'picturefield'.
        $fileindex = array_values(array_map(fn($file) => $file->index,
            array_filter($ziparchive->list_files(), fn($file) => $file->pathname === 'files/samplepicture.png')))[0];
        $filestream = $ziparchive->get_stream($fileindex);
        $filefield = datafos_get_field_from_name('picturefield', $data);
        $filefieldfilecontent = fread($filestream, $ziparchive->get_info($fileindex)->size);
        $this->assertEquals($filefield->get_file(array_keys($records)[0])->get_content(),
            $filefieldfilecontent);
        fclose($filestream);
    ...
    /**
     * Returns the records of the datafos instance.
     *
     ...
     *
     * @covers \mod_datafos\local\importer\csv_entries_importer::import_csv
     * @covers \mod_datafos\local\importer\csv_entries_importer::get_added_records_messages
    ...
    public function test_get_added_record_messages(string $datafilecontent, int $expectedcount): void {
        ...
        // First we need to create the zip file from the provided datafos.
     ...
     *
     * @return array datafos for testing
     */
