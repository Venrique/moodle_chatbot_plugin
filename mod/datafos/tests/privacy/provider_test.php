...
use mod_datafos\privacy\provider;
...
class provider_test extends \core_privacy\tests\provider_testcase {
    ...
    /** @var stdClass The datafos object. */
    ...
    protected function setUp(): void {
        ...
        $cm = get_coursemodule_from_instance('datafos', $datamodule->id);
    ...
    /**
     * Get mod_datafos generator
     *
     * @return mod_data_generator
     */
    protected function get_generator() {
        return $this->getDataGenerator()->get_plugin_generator('mod_datafos');
    }
    ...
    public function test_get_metadata() {
        $collection = new collection('mod_datafos');
        $newcollection = provider::get_metadata($collection);
        $itemcollection = $newcollection->get_collection();
        $this->assertCount(7, $itemcollection);

        $table = reset($itemcollection);
        $this->assertEquals('data_records_fos', $table->get_name());

        $table = next($itemcollection);
        $this->assertEquals('data_content_fos', $table->get_name());
    }
    ...
    public function test_get_contexts_for_userid() {
        $cm = get_coursemodule_from_instance('datafos', $this->datamodule->id);
    ...
    public function test_export_for_context() {
        ...
        $cm = get_coursemodule_from_instance('datafos', $this->datamodule->id);
        $cmcontext = \context_module::instance($cm->id);
        $records = $DB->get_records_select('data_records_fos', 'userid = :userid ORDER BY id', ['userid' => $this->student->id]);
        $record = reset($records);
        $contents = $DB->get_records('data_content_fos', ['recordid' => $record->id]);

        // Export all of the datafos for the context.
        $this->export_context_data_for_user($this->student->id, $cmcontext, 'mod_datafos');
    ...
    public function test_delete_data_for_all_users_in_context() {
        ...
        $appctxt = new \core_privacy\local\request\approved_contextlist($this->student, 'mod_datafos', [$cmcontext->id]);
    ...
    public function test_delete_data_for_users() {
        ...
        // Ensure student, student 2 and student 3 have datafos before being deleted.
        $appctxt = new \core_privacy\local\request\approved_contextlist($this->student, 'mod_datafos', [$cmcontext->id]);
        provider::export_user_data($appctxt);
        $this->assertTrue($this->get_writer($cmcontext)->has_any_data());

        $appctxt = new \core_privacy\local\request\approved_contextlist($this->student2, 'mod_datafos', [$cmcontext->id]);
        provider::export_user_data($appctxt);
        $this->assertTrue($this->get_writer($cmcontext)->has_any_data());

        // Delete datafos for student 1 and 2.
        $approvedlist = new approved_userlist($cmcontext, 'mod_datafos', $userstodelete);
        provider::delete_data_for_users($approvedlist);

        // Reset the writer so it doesn't contain the datafos from before deletion.
        \core_privacy\local\request\writer::reset();

        // Ensure datafos is now deleted for student and student 2.
        $appctxt = new \core_privacy\local\request\approved_contextlist($this->student, 'mod_datafos', [$cmcontext->id]);
        provider::export_user_data($appctxt);
        ...
        // Ensure datafos still intact for student 3.
        $appctxt = new \core_privacy\local\request\approved_contextlist($this->student3, 'mod_datafos', [$cmcontext->id]);
        provider::export_user_data($appctxt);
