...
/**
 * Contains unit tests for mod_datafos\dates.
 *
 * @package   mod_datafos
 * @category  test
...
/**
 * Class for unit testing mod_datafos\dates.
 *
 * @copyright 2021 Shamim Rezaie <shamim@moodle.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class dates_test extends advanced_testcase {
    ...
    public function test_get_dates_for_module(?int $availablefrom, ?int $availableto, array $expected) {
        ...
        $moddata = $this->getDataGenerator()->create_module('datafos', $data);

        $this->setUser($user);

        $cm = get_coursemodule_from_instance('datafos', $moddata->id);
