...
/**
 * Unit tests for datafos_get_all_recordsids(), datafos_get_advance_search_ids(), datafos_get_record_ids(),
 * and datafos_get_advanced_search_sql()
 *
 * @package    mod_datafos
 * @category   phpunit
...
require_once($CFG->dirroot . '/mod/datafos/tests/generator/lib.php');

/**
 * Unit tests for {@see datafos_get_all_recordids()}.
 *                {@see datafos_get_advanced_search_ids()}
 *                {@see datafos_get_record_ids()}
 *                {@see datafos_get_advanced_search_sql()}
 *
 * @package    mod_datafos
 * @copyright  2012 Adrian Greeve
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class search_test extends \advanced_testcase {
    /**
     * @var stdObject $recorddata An object that holds information from the table datafos.
     */
    ...
    protected function setUp(): void {
        ...
        $this->databaseentryareaid = \core_search\manager::generate_areaid('mod_datafos', 'entry');
    ...
    /**
     * Test 1: The function datafos_get_all_recordids.
     *
     * Test 2: This tests the datafos_get_advance_search_ids() function. The function takes a set
     ...
     *
     * Test 3: This function tests datafos_get_recordids(). This is the function that is nested in the last
     * function (see datafos_get_advance_search_ids). This function takes a couple of
     ...
     *
     * Test 4: datafos_get_advanced_search_sql provides an array which contains an sql string to be used for displaying records
     ...
     *
     * Test 5: Returning to datafos_get_all_recordids(). Here we are ensuring that the total amount of record ids is reduced to
     ...
     *
     * Test 6: datafos_get_all_recordids() again. This time we are testing approved database records. We only want to
    ...
    public function test_advanced_search_sql_section() {
        ...
        // TODO: Invent a better way for managing datafos file input against database sequence id's.
        ...
        // Set up datafos for the test database.
        $files = array(
                'data_fields_fos'  => __DIR__.'/../fixtures/test_data_fields.csv',
                'data_records_fos' => __DIR__.'/../fixtures/test_data_records.csv',
        ...
        $fieldinfo = array('0' => new \stdClass(),
                ...
                '4' => new \stdClass());
        $fieldinfo['0']->id = 1;
        $fieldinfo['0']->datafos = '3.721,46.6126';
        $fieldinfo['1']->id = 2;
        $fieldinfo['1']->datafos = 'Hahn Premium';
        $fieldinfo['2']->id = 5;
        $fieldinfo['2']->datafos = 'Female';
        $fieldinfo['3']->id = 7;
        $fieldinfo['3']->datafos = 'kel';
        $fieldinfo['4']->id = 9;
        $fieldinfo['4']->datafos = 'VIC';
        ...
        // Test 1
        $recordids = datafos_get_all_recordids($this->recorddata->id);
        $this->assertEquals(count($recordids), $this->datarecordcount);

        // Test 2
        $key = array_keys($this->recordsearcharray);
        $alias = $key[0];
        $newrecordids = datafos_get_recordids($alias, $this->recordsearcharray, $this->recorddata->id, $recordids);
        $this->assertEquals($this->datarecordset, $newrecordids);

        // Test 3
        $newrecordids = datafos_get_advance_search_ids($recordids, $this->recordsearcharray, $this->recorddata->id);
        $this->assertEquals($this->datarecordset, $newrecordids);

        // Test 3.1
        $resultrecordids = datafos_get_advance_search_ids(array(), $this->recordsearcharray, $this->recorddata->id);
        ...
        $recordids = datafos_get_all_recordids($this->recorddata->id, $groupsql, $params);
        $this->assertEquals($this->groupdatarecordcount, count($recordids));

        // Test 6
        $approvesql = ' AND r.approved=1 ';
        $recordids = datafos_get_all_recordids($this->recorddata->id, $approvesql, $params);
    ...
    public function test_advanced_search_tags() {
        ...
        $searcharray[DATAFOS_TAGS]->rawtagnames = ['Cats'];
        $searcharray[DATAFOS_TAGS]->sql = '';

        $recordids = datafos_get_all_recordids($data1->id);
        $newrecordids = datafos_get_advance_search_ids($recordids, $searcharray, $data1->id);
    ...
    public function test_data_entries_indexing() {
        ...
        $this->assertInstanceOf('\mod_datafos\search\entry', $searcharea);
        ...
        $data1 = $this->getDataGenerator()->create_module('datafos', $record);
        ...
        $data2 = $this->getDataGenerator()->create_module('datafos', ['course' => $course1->id]);
    ...
    public function test_data_entries_document() {
        ...
        // Second Case.
        $data2 = $this->getDataGenerator()->create_module('datafos', $record);
        ...
        // Third Case.
        $data3 = $this->getDataGenerator()->create_module('datafos', $record);
        ...
        // Fourth Case.
        $data4 = $this->getDataGenerator()->create_module('datafos', $record);
        ...
        // Fifth Case.
        $data5 = $this->getDataGenerator()->create_module('datafos', $record);
        ...
        // Sixth Case.
        $data6 = $this->getDataGenerator()->create_module('datafos', $record);
        ...
        // Seventh Case.
        $data7 = $this->getDataGenerator()->create_module('datafos', $record);
        ...
        // Eight Case.
        $data8 = $this->getDataGenerator()->create_module('datafos', $record);
        ...
        // Ninth Case.
        $data9 = $this->getDataGenerator()->create_module('datafos', $record);
        ...
        // Tenth Case.
        $data10 = $this->getDataGenerator()->create_module('datafos', $record);
    ...
    /**
     * Group support for datafos entries.
     */
    public function test_data_entries_group_support() {
        ...
        $data = self::getDataGenerator()->create_module('datafos', ['course' => $course->id,
    ...
    public function test_data_entries_access() {
        ...
        $cm = get_coursemodule_from_instance('datafos', $data2->id, $course->id);
        $cm->groupmode = '1';
        ...
        $DB->update_record('datafos', array('id' => $data3->id, 'approval' => 1));
        ...
        $DB->update_record('data_records_fos', array('id' => $data3record1id, 'approved' => 1));
    ...
    public function test_attach_files() {
        ...
        $data1filefieldid = $DB->insert_record('data_fields_fos', $record);
        ...
        $data1textfieldid = $DB->insert_record('data_fields_fos', $record);
        ...
        $data1textareafieldid = $DB->insert_record('data_fields_fos', $record);
        ...
        $data1contentfileid1 = $DB->insert_record('data_content_fos', $record);
        ...
        $DB->insert_record('data_content_fos', $record);
        ...
        $data1contentfileid2 = $DB->insert_record('data_content_fos', $record);
    ...
    protected function create_default_data_fields($fieldtypes, $data) {
        ...
        foreach ($fieldtypes as $fieldtype) {
            ...
            ${$fieldname} = $this->getDataGenerator()->get_plugin_generator('mod_datafos')->create_field($record, $data);
            $count++;
        }
    }
    ...
    protected function create_default_data_record($data, $groupid = 0) {
        ...
        return $this->getDataGenerator()->get_plugin_generator('mod_datafos')->create_entry($data, $fieldcontents, $groupid);
    }
