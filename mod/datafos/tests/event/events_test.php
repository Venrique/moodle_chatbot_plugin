...
class events_test extends \advanced_testcase {
    ...
    public function test_field_created() {
        ...
        // Create a course we are going to add a datafos module to.
        $course = $this->getDataGenerator()->create_course();

        // The generator used to create a datafos module.
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_datafos');

        // Create a datafos module.
        ...
        // Check that the event datafos is valid.
        $this->assertInstanceOf('\mod_datafos\event\field_created', $event);
        $this->assertEquals(\context_module::instance($data->cmid), $event->get_context());
        $url = new \moodle_url('/mod/datafos/field.php', array('d' => $data->id));
    ...
    public function test_field_updated() {
        ...
        // Check that the event datafos is valid.
        $this->assertInstanceOf('\mod_datafos\event\field_updated', $event);
        $this->assertEquals(\context_module::instance($data->cmid), $event->get_context());
        $url = new \moodle_url('/mod/datafos/field.php', array('d' => $data->id));
        $this->assertEquals($url, $event->get_url());
    }
    ...
    public function test_field_deleted() {
        ...
        // Check that the event datafos is valid.
        $this->assertInstanceOf('\mod_datafos\event\field_deleted', $event);
        $this->assertEquals(\context_module::instance($data->cmid), $event->get_context());
        $url = new \moodle_url('/mod/datafos/field.php', array('d' => $data->id));
        $this->assertEquals($url, $event->get_url());
    }
    ...
    public function test_record_created() {
        ...
        $sink = $this->redirectEvents();
        $recordid = datafos_add_record($data);
        $events = $sink->get_events();
        $event = reset($events);

        // Check that the event datafos is valid.
        $this->assertInstanceOf('\mod_datafos\event\record_created', $event);
        $this->assertEquals(\context_module::instance($data->cmid), $event->get_context());
        $url = new \moodle_url('/mod/datafos/view.php', array('d' => $data->id, 'rid' => $recordid));
        $this->assertEquals($url, $event->get_url());
    }
     ...
     * and trigger the event and ensure the legacy log datafos is returned as expected.
     */
    public function test_record_updated() {
        ...
        // Trigger and capture the event for updating the datafos record.
        $sink = $this->redirectEvents();
        $event->trigger();
        $events = $sink->get_events();
        $event = reset($events);
        ...
        // Check that the event datafos is valid.
        $this->assertInstanceOf('\mod_datafos\event\record_updated', $event);
        $this->assertEquals(\context_module::instance($data->cmid), $event->get_context());
        $url = new \moodle_url('/mod/datafos/view.php', array('d' => $data->id, 'rid' => $event->objectid));
        $this->assertEquals($url, $event->get_url());
    }
    ...
    public function test_record_deleted() {
        ...
        $datacontent->id = $DB->insert_record('data_content_fos', $datacontent);

        // Trigger and capture the event for deleting the datafos record.
        $sink = $this->redirectEvents();
        datafos_delete_record($datarecords->id, $data, $course->id, $data->cmid);
        $events = $sink->get_events();
        $event = reset($events);

        // Check that the event datafos is valid.
        $this->assertInstanceOf('\mod_datafos\event\record_deleted', $event);
        $this->assertEquals(\context_module::instance($data->cmid), $event->get_context());
        $url = new \moodle_url('/mod/datafos/view.php', array('d' => $data->id));
        $this->assertEquals($url, $event->get_url());
    }
    ...
    public function test_template_viewed() {
        ...
        // Trigger an event for updating this record.
        $event = \mod_datafos\event\template_viewed::create(array(
            'context' => \context_module::instance($data->cmid),
            'courseid' => $course->id,
            'other' => array(
                'datafosid' => $data->id
        ...
        // Check that the event datafos is valid.
        $this->assertInstanceOf('\mod_datafos\event\template_viewed', $event);
        $this->assertEquals(\context_module::instance($data->cmid), $event->get_context());
        $url = new \moodle_url('/mod/datafos/templates.php', array('d' => $data->id));
        $this->assertEquals($url, $event->get_url());
    }
    ...
    public function test_template_updated() {
        ...
        // Trigger an event for updating this record.
        $event = \mod_datafos\event\template_updated::create(array(
            'context' => \context_module::instance($data->cmid),
            'courseid' => $course->id,
            'other' => array(
                'datafosid' => $data->id,
        ...
        // Check that the event datafos is valid.
        $this->assertInstanceOf('\mod_datafos\event\template_updated', $event);
        $this->assertEquals(\context_module::instance($data->cmid), $event->get_context());
        $url = new \moodle_url('/mod/datafos/templates.php', array('d' => $data->id));
        $this->assertEquals($url, $event->get_url());
    }
    ...
    public function test_importing_events(
        ...
        $this->resetAfterTest();
        $this->setAdminUser();
        $plugingenerator = $this->getDataGenerator()->get_plugin_generator('mod_datafos');
        ...
        foreach ($expected as $triggeredevent => $count) {
            for ($i = 0; $i < $count; $i++) {
                ...
                // Check that the event datafos is valid.
                $this->assertInstanceOf('\mod_datafos\event\\'.$triggeredevent, $event);
                $this->assertEquals(\context_module::instance($activity->cmid), $event->get_context());
                $this->assertEventContextNotUsed($event);
                $url = new \moodle_url('/mod/datafos/field.php', ['d' => $activity->id]);
                $this->assertEquals($url, $event->get_url());
            }
        }
    }
