...
/**
 * PHPUnit datafos generator testcase.
 *
 * @package    mod_datafos
 * @category   phpunit
...
class generator_test extends \advanced_testcase {
    ...
    public function test_generator() {
        ...
        $this->assertEquals(0, $DB->count_records('datafos'));
    ...
    public function test_create_field() {
        ...
        $data = $generator->create_instance(['course' => $course->id]);
        $this->assertEquals(1, $DB->count_records('datafos'));
    ...
    public function test_create_entry() {
        ...
        foreach ($fieldtypes as $fieldtype) {
            ...
            $this->getDataGenerator()->get_plugin_generator('mod_datafos')->create_field($record, $data);
            $count++;
        }
        ...
        $this->setUser($user1);
        $datarecordid = $this->getDataGenerator()->get_plugin_generator('mod_datafos')->create_entry(
            $data,
            $fieldcontents,
            $groupa->id,
            $tags
        ...
        $contents = $DB->get_records('data_content_fos', ['recordid' => $datarecordid], 'id');
        ...
        $this->assertEquals(
            ['Cats', 'mice'],
            array_values(\core_tag_tag::get_item_tags_array('mod_datafos', 'data_records_fos', $datarecordid))
        );
    }
     ...
     * @param stdClass|null $record datafos for the preset that will be created (like name or description)
     */
