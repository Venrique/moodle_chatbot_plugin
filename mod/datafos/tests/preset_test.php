...
/**
 * Preset tests class for mod_datafos.
 *
 * @package    mod_datafos
 * @category   test
 * @copyright  2022 Sara Arjona <sara@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @coversDefaultClass \mod_datafos\preset
 */
class preset_test extends \advanced_testcase {
    ...
    public function test_export() {
        ...
        foreach (manager::TEMPLATES_LIST as $templatename => $notused) {
            datafos_generate_default_template($activity, $templatename);
        }
    ...
    public function is_directory_a_preset_provider(): array {
        ...
        return [
            'Valid preset directory' => [
                'directory' => $CFG->dirroot . '/mod/datafos/preset/imagegallery',
                'expected' => true,
            ],
            'Invalid preset directory' => [
                'directory' => $CFG->dirroot . '/mod/datafos/field/checkbox',
                'expected' => false,
            ],
    ...
    public function test_can_manage() {
        ...
        $course = $this->getDataGenerator()->create_course();
        $data = $this->getDataGenerator()->create_module('datafos', ['course' => $course->id]);
